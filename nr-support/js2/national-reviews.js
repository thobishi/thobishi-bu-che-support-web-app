...
function checkDocumentsSelected (question, priv_publ) {
	// 2010-01-05 Robin: Replacing document.All to be compatible with Firefox.
	// obj = document.All;
	// I don't think that this function is used anymore even though it is called from +- 30 accForm templates.  It was relevant when users had the option of posting the document and indicating it by means of the radio button.
	obj = document.getElementById('defaultFrm');
...
function openFileWin(page,val,field){
	...
	$fileUploader
		...
		.end()
		.children('.modal-body')
			.html('<iframe id="uploadFrame" name="uploadFrame" src="' + $url + '"></iframe>')
		.end()
		.modal($options);
}

function showProgressBar(progressUrl, title) {
	var $fileUploader = $('#fileUploader');

	$fileUploader
		.children('.modal-body')
			.find('iframe').hide().end()
			.append('<iframe id="progressBarFrame" name="progressBarFrame" src="' + progressUrl + '"></iframe>');	
}
...
function loginAttemptsError(message){
	$('#infoModal > .modal').attr('tabindex', -1);
	$('#infoModal').css({width: '640px', height: 'auto'});
	$('#infoModal > .modal-header > h3').html('Login Error');
	$('#infoModal > .modal-body').html(message);
	$('#infoModal').modal({keyboard: true});
}

function showProcessDescription(id){
	var url= "pages/ProcessDescription.php?id="+id;
	$.get(url, function(data) {
		$('#infoModal > .modal').attr('tabindex', -1);
		$('#infoModal').css({width: '700px', height: 'auto'});
		$('#infoModal > .modal-header > h3').html('Help');
		$('#infoModal > .modal-body').html(data);
		$('#infoModal').modal({keyboard: true});
	});
}
function showSERreadOnly(prog_id){
	var url= "pages/ser_read_only.php?prog_id="+prog_id;
	var pdfUrl = "docgen/pdf_online_table.php?prog_id="+prog_id;
	$.get(url, function(data) {
		$('#infoModal').attr('tabindex', -1);
		$('#infoModal > .modal-body').addClass('previewModalBody');
		$('#infoModal > .modal-header > h3').addClass('previewModalHeader');
		$('#infoModal').addClass('previewModal');
		$('#infoModal').modal({keyboard: true});
		$('#infoModal > .modal-body').attr('id', 'modal-body');
		$('#infoModal > .modal-header > h3').html('Preview online tables');

		$('#infoModal > .modal-body').html(data);
		$('#infoModal > .modal-footer > a').html("Close");	
		$('#infoModal > .modal-header .print').removeClass('hidden');
		$('#infoModal > .modal-body').html(data);
		$('#infoModal > .modal-footer > a').html("Close");			
		$('#infoModal > .modal-footer > a').click(function(){
			$('#infoModal > .modal-header .print').addClass('hidden');
		});
		$('#infoModal > .modal-header > button').click(function(){
			$('#infoModal > .modal-header .print').addClass('hidden');
		});		
	});
	$('#infoModal > .modal-header .print').click(function(){
		$('#infoModal > .modal-header .pdfDownloadBtn').attr('href',pdfUrl);

	});
}

function recommCriteriaComparison(prog_id, ModalTitle){
	if(typeof ModalTitle === 'undefined'){
		ModalTitle = 'SER and Review panel criteria compared';
	}
	
	var url= "pages/recommCriteriaComparison.php?prog_id="+prog_id;
	// var pdfUrl = "docgen/pdf_ser_panel_criteria_comparison.php?prog_id="+prog_id;
	$.get(url, function(data) {
			$('#infoModal > .modal').attr('tabindex', -1);
			$('#infoModal > .modal-body').addClass('previewModalBody');
			$('#infoModal > .modal-header > h3').addClass('previewModalHeader');
			$('#infoModal').addClass('previewModal');
			$('#infoModal > .modal-header > h3').html(ModalTitle);
			$('#infoModal > .modal-body').html(data);
			$('#infoModal').modal({keyboard: true});
		// $('#infoModal').attr('tabindex', -1);
		// $('#infoModal > .modal-body').addClass('previewModalBody');
...
function progressReportOfNR(prog_id, progName){
	var url= "pages/progressReportOfNR.php?prog_id="+prog_id;
	// var pdfUrl = "docgen/pdf_ser_panel_criteria_comparison.php?prog_id="+prog_id;
	$.get(url, function(data) {
			$('#infoModal > .modal').attr('tabindex', -1);
			$('#infoModal > .modal-body').addClass('previewModalBody');
			$('#infoModal > .modal-header > h3').addClass('previewModalHeader');
			$('#infoModal').addClass('previewModal');
			$('#infoModal > .modal-header > h3').html("National Reviews | "+progName);
			$('#infoModal > .modal-body').html(data);
			$('#infoModal').modal({keyboard: true});
		// $('#infoModal').attr('tabindex', -1);
		// $('#infoModal > .modal-body').addClass('previewModalBody');
...
function winContentText(title, template, content) {
	...
	switch (content) {
		case "help_home_page" : 
			...
			content += '<li><b>Tools:</b> allows access to your institutional profile; adding/editing colleague details and useful downloads. </li>';
			content += '<li><b>Reports:</b> displays the application progress report for your institution (which allows read-only access to all applications - both submitted and in progress).</li>';
			...
			content += '<li><b>Home</b>: allows you to go back to the Home page with a list of your active processes.</li>';
			content += '<li><b>Help:</b> general help for the accreditation system</li>';
			content += '<li><b>Log out:</b> allows you to conclude a task and exit the system from any given screen</li>';
			content += '</ul>';
			content += '</span>';		
...
function winPrintReaccApplicForm(title, appid, workflow_settings,path) {
	if (applicationwin && applicationwin.open && !applicationwin.closed){
		applicationwin.close();
	}
	w=(screen.width-100)+"px";
...
//	applicationwin = window.showModelessDialog(path+"pages/printApplicationFormFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&appid="+appid,"","dialogWidth:"+w+"; dialogHeight:"+h+"; dialogTop:"+topSide+"; dialogLeft:"+leftSide+"; resizable:yes; scroll:yes;center:no");
	//applicationwin = open(path+"pages/printApplicationFormFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&appid="+appid,"","dialogWidth:"+w+"; dialogHeight:"+h+"; dialogTop:"+topSide+"; dialogLeft:"+leftSide+"; resizable:yes; scroll:yes;center:no");
	applicationwin = open(path+"pages/printReaccApplicFormFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&appid="+appid,"","width="+w+"; height="+h+"; top="+topSide+"; left="+leftSide+"; resizable=1; scroll:yes;center:no");
}

function winPrintReaccProcessInfo(title, appid, workflow_settings,path) {
	if (applicationwin && applicationwin.open && !applicationwin.closed){
		applicationwin.close();
	}
	w=(screen.width-100)+"px";
	...
	leftSide = ((screen.width-w)/1.3);
	topSide = 25;
	applicationwin = open(path+"pages/printReaccApplicProcessFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&appid="+appid,"","width="+w+"; height="+h+"; top="+topSide+"; left="+leftSide+"; resizable=1; scroll:yes;center:no");
}
...
function winPrintInstProfileForm(title, appid, workflow_settings, path) {
	...
	leftSide = ((screen.width-w)/1.3);
	topSide = 15;
//	instprofilewin = window.showModelessDialog(path+"pages/printInstProfileFormFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&evalReport_id="+appid,"","dialogWidth:"+w+"; dialogHeight:"+h+"; dialogTop:"+topSide+"; dialogLeft:"+leftSide+"; resizable:yes; scroll:yes;center:no");
//	instprofilewin = open(path+"pages/printInstProfileFormFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&evalReport_id="+appid,"","dialogWidth:"+w+"; 	dialogHeight:"+h+"; 	dialogTop:"+topSide+"; 	dialogLeft:"+leftSide+"; resizable:1; scroll:yes;center:no");
	instprofilewin = open(path+"pages/printInstProfileFormFrames.php?workflow_settings="+workflow_settings+"&title="+title+"&evalReport_id="+appid,"","width="+w+"; 	height="+h+"; 		top="+topSide+"; 	left="+leftSide+"; 	resizable=1; 	scroll:yes;center:no");
...
// the fireEvent doesn't work in IE5
		if (document.fireEvent) {
			...
			// firfox
			var go = true;
			if (document.defaultFrm.onsubmit) {
				go = document.defaultFrm.onsubmit();
			}
			if (go) {
				document.defaultFrm.submit();
			}
...
// 2008-01-03 Robin: Added to allow for cancel of changes without workflow settings being cleared.
...
function winEvalContactDetails(title, userid, workflow_settings,path) {
	if (applicationwin && applicationwin.open && !applicationwin.closed){
		applicationwin.close();
	}
	w=(screen.width-500)+"px";
	h=(screen.height-800)+"px";
	leftSide = ((screen.width-w)/1.3);
	topSide = 25;

	detailswin = open(path+"pages/printEvalContactDetails.php?workflow_settings="+workflow_settings+"&title="+title+"&userid="+userid,"","width="+w+"; height="+h+"; top="+topSide+"; left="+leftSide+"; resizable=1; scroll=yes;center=no");
}

//2008-01-02::Added by Rebecca to this file as same function is used in multiple templates - need to update functions with function still "hardcoded"
function changeCMD (newCMD) {
	document.defaultFrm.cmd.value = newCMD;
}

function setRequest(val){
	document.defaultFrm.CHANGE_TO_RECORD.value='appTable_requests|'+val;
	moveto('next');
}
...
function setAdditionalDocs(val){
	document.defaultFrm.CHANGE_TO_RECORD.value='nr_programme_additional_docs|'+val;
	moveto('_label_ser_prelim_upload_additional_docs');
}
function addMeeting(val){
	document.defaultFrm.CHANGE_TO_RECORD.value='nr_meetings|'+val;
	moveto('_label_admin_addMeetings');
}

function addRGMeeting(val){
	document.defaultFrm.CHANGE_TO_RECORD.value='rg_meetings|'+val;
	moveto('_label_admin_addRGMeetings');
}

function addUser(val){
	document.defaultFrm.CHANGE_TO_RECORD.value='users|'+val;
	moveto('_admin_addUser');
}
//general function to add new data to a table an move to label
//thee disadvantage is that it exposes the table
function addNewfieldsInTable(val, DBtable, label){
	document.defaultFrm.CHANGE_TO_RECORD.value = DBtable +'|'+val;
	moveto(label);
}
// 20080221: Diederik removed this from the index page where it was generated as part of the page HEAD	
...
function popupWindow(title, html) {
	...
	helpwin.document.write('<link rel=STYLESHEET TYPE="text/css" href="styles.css" title="Normal Style">');
...
function displayIfNo(value, div_id) {
	if (value == 1) {
...
// Robin 2009-08-06
function displaybyOutcome(value) {
	if (value == 4) {
		document.getElementById("deferral").style.display="block";
	}
	else {
		document.getElementById("deferral").style.display="none";
	}
	if (value == 2) {
...
//2008-02-27: Rebecca - displays a div if value of dropdown ("value") is "yes"
function displayYesNo(value, div_id) {
	if (value == 2) {
		document.getElementById(div_id).style.display="block";
	}
	else {
...
function displayIfQual(value, div_id) {
	if (value == 5 || value == 6 || value == 8 || value == 9 || value == 10) {
		document.getElementById(div_id).style.display="block";
	}
	else {
		document.getElementById(div_id).style.display="none";
		// Reset fields to no value.
		var fld = document.getElementsByName('FLD_qualification_designator_ref');
		fld[0].selectedIndex = "0";
		
		// OTher div must also be reset
		document.getElementById('is_other').style.display="none";
		// Reset fields to no value.
		var fld1 = document.getElementsByName('FLD_designation');
		fld1[0].value = "";
		var fld2 = document.getElementsByName('FLD_motivation_alt_designator');
		fld2[0].value = "";
	}

	// 2010-01-07 Commented out until HEQC can resolve how it must work.
	//document.getElementById('nqf5').style.display="none";
	//document.getElementById('nqf6').style.display="none";	
	//document.getElementById('nqf7').style.display="none";
	//document.getElementById('nqf8').style.display="none";
	//document.getElementById('nqf9').style.display="none";
	//document.getElementById('nqf10').style.display="none";

	//if (value == 1 || value == 2 || value == 3 || value == 5 || value == 6) {	document.getElementById('nqf5').style.display="block";	}
	//if (value == 1 || value == 2 || value == 3 || value == 4 || value == 5 || value == 6) {	document.getElementById('nqf6').style.display="block";	}
	//if (value == 2 || value == 3 || value == 4 || value == 5 || value == 6 || value == 7 || value == 8) {	document.getElementById('nqf7').style.display="block";	}
	//if (value == 4 || value == 6 || value == 7 || value == 8 || value == 9) {	document.getElementById('nqf8').style.display="block";	}
	//if (value == 7 || value == 8 || value == 9 || value == 10) {	document.getElementById('nqf9').style.display="block";	}
	//if (value == 9 || value == 10) {	document.getElementById('nqf10').style.display="block";	}
...
function displayIfPending(value, div_id) {
	if (value == 3) {
		document.getElementById(div_id).style.display="block";
	}
	else {
...
function checkall(chkarr,chk){
	sLength = chkarr.length;
	for (i=0; i<sLength; i++) {
		chkarr[i].checked = chk;
	}
}
...
function valDocRequired(fld,msg){
	if (fld.value == 0 || isNaN(fld.value)) {
		alert(msg);
		document.defaultFrm.MOVETO.value = '';
		return false;
...
function valDateRequired(fld,msg){
	if (fld.value == '1000-01-01' || fld.value == '') {
		alert(msg);
		fld.focus();
		document.defaultFrm.MOVETO.value = '';
		return false;
...
function valSelectRequired(fld,msg){
...
function valRadioRequired(fld,msg) {
    var cnt = -1;

	if (fld.length == undefined){
		if (fld.checked) {cnt = 1;}
	} else {
...
function valCheckboxRequired(fld,msg) {
    if (!fld.checked) {
		alert(msg);
    	return false;
	}
...
function valCheckboxArrayRequired(fld,msg) {
	var len = fld.length;
	for(var i=0; i < len; i++){
		if (fld[i].checked) {
			return true;
...
// 2010-07-30 Robin
function valTelNo(fld){
	var str = fld.value.substr(0,2);
	if (str.substr(0,1) != 0) {
		alert('Please ensure that the contact telephone number starts with an area code.');
		document.defaultFrm.MOVETO.value = '';
		fld.value='';
		fld.focus();
		return false;
	}
	if (str.substr(1,1) == '7' || str.substr(1,1) == '8') {
		alert('Please enter your cellphone number in Contact Cell No.');
		document.defaultFrm.MOVETO.value = '';
		fld.value='';
		fld.focus();
		return false;
...
// 2010-07-30 Robin
function valCellNo(fld){
	var cellstr = fld.value.substr(0,2);
	if (cellstr.substr(0,1) != 0) {
		alert('Please ensure that the contact cell number starts with 0.');
		document.defaultFrm.MOVETO.value = '';
		fld.value='';
		fld.focus();
		return false;
	}
	if (!(cellstr.substr(1,1) == '7' || cellstr.substr(1,1) == '8')) {
		alert('Please enter a valid contact cellphone number.');
		document.defaultFrm.MOVETO.value = '';
		fld.value='';
		fld.focus();
		return false;
	}
	if (!(fld.value.length > 9)) {//contactCellNo
		alert('Please enter a valid contact cellphone number');
		document.defaultFrm.MOVETO.value = '';
		fld.focus();
		return false;
...
//2010-01-05 Robin
function displayCESMLevel2(value, formObj, formObj1){

	curMuns = a_cesmlevel2[value];

	idx = formObj.selectedIndex;
	...
	displayCESMLevel3(value, formObj1)
}

//2010-01-05 Robin
function displayCESMLevel3(value, formObj){

	curMuns = a_cesmlevel3[value];
...
function displayCESMLevel3Defn(){
	idx = document.defaultFrm.FLD_CESM_level3_ref.selectedIndex;

	if (idx > 0){
		val = document.defaultFrm.FLD_CESM_level3_ref.options[idx].value;
		document.defaultFrm.CESM_level3_defn.value = a_cesmlevel3_defn[val];
	}
}

function clearForm(oFld) {
    
  field_type = oFld.type.toLowerCase();
...
function isMatch (str) {
	var regexp = /^[A-z0-9_\-\.]+[@]{1}[A-z0-9_\-]+([.][A-z0-9_\-]+){1,3}$/;
	return regexp.test(str);
}

var insertCount = 1;

function generateTemplate($parentRow, $deleteClass) {
	var $this = $(this);
	...
	var $delLink = $template.find('a.' + $deleteClass);
...
function generateTemplateTable($rowToClone, $deleteClass) {
	var $this = $(this);
	$template = $rowToClone.clone();
	$template.find(':input').each(function() {
		var $_this = $(this);
		var insertId = 'INSERT' + insertCount;
		var inputName = $_this.attr('name')
...
function validation($validationData, $value){
	$return = {
		'error': false
	};
	
	$regExp = eval($validationData['fieldValidationRegExp']);
	$message = $validationData['fieldValidationDesc'];
	$field = $validationData['fieldDisplayName'];
...
$(function(){
	var rowDeleteClassMap = new Array();
		rowDeleteClassMap['tableByRowAdd'] = 'tableByRowDel';
		rowDeleteClassMap['rowByRowAdd'] = 'rowByRowDel';
	var $delete_moveTo = '',
		$delete_record = '',
		$delete_field = '';
		
	$('a.tooltipImg').tooltip();
	

	$('.rowByRowAdd').on('click', function(e) {
		var $this = $(this);
		var $parentRow = $this.closest('tr');
		var $rowTemplate = generateTemplate.call(this, $parentRow, rowDeleteClassMap[$this.attr("class")]);	
	...
	$('.tableByRowAdd').on('click', function(e) {
		e.preventDefault();
		var $this = $(this);
		var $rowToClone = $this.closest('tr').prev('tr').find('td:eq(0)').find('table:last()');
		var $rowTemplate = generateTemplateTable.call(this, $rowToClone, rowDeleteClassMap[$this.attr("class")]);	
		
		if ($rowTemplate === false) {
			return true;
		}

		$rowToClone.after($rowTemplate);
	});

	$(".table tbody").on("click", ".rowByRowDel", function(e){
		e.preventDefault();
	...
	$(".table tbody").on("click", ".tableByRowDel", function(e){
		e.preventDefault();
	...
	$(".panel").click(function(e){
		...
		$href = $hrefParts[0];
		e.preventDefault();
		$validationResults = new Array();
		$validationDisplay = '';
		$validationDisplayRows = '';
		$showModal = false;
		$options = {
			'keyboard': true,
			'width': 300,
		...
		if(typeof $hrefParts[1] != 'undefined' && $hrefParts[1] == 'off'){
		...
		$('.validate').each(function(){			
			$validationData = $(this).data('validation');
			if($validationData > ''){
				$validationResults[$count] = validation($validationData, $(this).val());
				$count++;
		...
		if($validationResults > ''){
			for($number in $validationResults){
				if($validationResults[$number].error){
					$showModal = true;
					if((typeof $validationResults[parseInt($number+1)] != 'undefined') &&  ($validationResults[$number].field == $validationResults[parseInt($number+1)].field)){
						$validationResults.splice($validationResults[parseInt($number+1)],1);
					}
					$validationDisplayRows += '<tr><td>' + $validationResults[$number].field + '</td><td>' + $validationResults[$number].message + '</td></tr>';
				}
			}
		}
		
		if($showModal){
			$validationDisplay = '<table class="table table-hover table-bordered table-striped">';
			$validationDisplay += '<thead><tr><th>Field</th><th>Error message</th></tr></thead>';
			$validationDisplay += '<tbody>';
			$validationDisplay += $validationDisplayRows;
			$validationDisplay += '</tbody>';
			$validationDisplay += '</table>';
			$('#infoModal > .modal-header > h3').html('Validation errors');
			$('#infoModal > .modal-body').html($validationDisplay);
			$('#infoModal').attr('tabindex', -1);
	...
	$(".deleteFile").click(function(e){
		...
		$delete_field = $(this).data('field');
		$delete_record = $(this).data('delete_record');
		$delete_moveTo = $(this).data('moveto');
		
		$options = {
			'keyboard': true
		};
		
		$('#infoModal > .modal-footer a').removeAttr('data-dismiss');
		$('#infoModal > .modal-footer a').addClass('deleteFileConfirm');
		$('#infoModal').attr('tabindex', -1);
		$('#infoModal > .modal-header > h3').html('DELETE FILE');
		$('#infoModal > .modal-body').html($delete_message);
		$('#infoModal').modal($options);
	});
	
	function deleteFileField(){
		$('input[name=' + $delete_field + ']').val(0);
		document.defaultFrm.DELETE_RECORD.value = $delete_record;
		moveto($delete_moveTo);
	}
	
	$(".modal").on("click", ".deleteFileConfirm", function(event){
		event.preventDefault();
		$('#infoModal').modal('hide');
		deleteFileField();
	});
	
	$(".multiGridDiv").clickNScroll({
		allowHiliting: true
	});
	
	$('.multiGridDiv').mousedown(function(event){
	...
	$('.numbers table input').keypress(function(e){
		var a = [],
			k = e.which;
	...
	$('form').on('keyup', '.numbers table input' , function(e){
	...
	$(".showHide").click(function(e){
		...
		$target = $('.' + $(this).data('target')).html();
		
		$($modal).attr('tabindex', -1);
		$($modal + ' > .modal-header > h3').html($title);
		$($modal + ' > .modal-body').html($target);
		$($modal).modal();
	});
