...
htmlRow;
	...
	function getRecommDetails($lkpField, $lkpValue){
		...
		while($row = $RS->fetch()){
			...
			$recomm['accessDates'] = ($row['recommendation_start_date'] == "1000-01-01" || $row['recommendation_end_date'] == "1000-01-01" ) ? "Not assigned" : $row['recommendation_start_date'] . " to " . $row['recommendation_end_date'];
			$recomm['due-date'] = ($row['recommendation_report_due_date'] == "1000-01-01") ? "Not assigned" : $row['recommendation_report_due_date'];
		}
		return $recomm;
	}	
	...
	function getPanelDetails($lkpField, $lkpValue, $panelStyle = ""){
		...
		$siteVisitSubmittedByAdmin_ind = $this->db->getValueFromTable("nr_programmes","id",$lkpValue,"siteVisitSubmittedByAdmin_ind");	
			...
			$panel['accessDates'] = ($row['panel_start_date'] == "1000-01-01" || $row['panel_end_date'] == "1000-01-01" ) ? "Not assigned" : $row['panel_start_date'] . " to " . $row['panel_end_date'];
			$panel['site_visit_date'] = (($row['site_visit_date'] == "1000-01-01") ? "Not assigned" : $row['site_visit_date']);
		}
		return $panel;		
	}
	...
	function getPrelimAnalysisDetails($lkpField, $lkpValue){
		...
		while($row = $RS->fetch()){
			...
			$prelim['accessDates'] = ($row['analyst_start_date'] == "1000-01-01" || $row['analyst_end_date'] == "1000-01-01" ) ? "Not assigned" : $row['analyst_start_date'] . " to " . $row['analyst_end_date'];
		}
		return $prelim;
	}
	...
	function getScreeningDetails($lkpField, $lkpValue,$form =""){
		...
		if(!empty($screening)){
			foreach($screening as $screen){
				$return .= $screen['screener'] . ', ' . (($screen['date_screening_signed'] != '1000-01-01') ? $screen['date_screening_signed'] : 'In progress') . ', ' . ((!empty($screen['link_report']) ? $screen['link_report'] : 'No report')) . ' <hr>';
			}
		}
