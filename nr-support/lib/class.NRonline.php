...
class NRonline extends miscellaneous {
	...
	var $reportSearchFields = array(
		...
		/*'national_standard_alignemnt_report_previ' => array(
			'nr_programme_name' => array('function' => 'filter_compare'),
			'nr_programme_abbr' => array('function' => 'filter_compare'),
			'heqsf_reference_no' => array('function' => 'filter_compare'),
			'nr_national_review_id' => array('function' => 'filter_equal',
		...
		'national_standard_alignemnt_report' => array(
			'nr_programme_name' => array('function' => 'filter_compare'),
			'nr_programme_abbr' => array('function' => 'filter_compare'),
			'heqsf_reference_no' => array('function' => 'filter_compare'),
			'nr_national_review_id' => array('function' => 'filter_equal',
 ...
 */
	...
	*/
	...
	function HemisData($prog_id){
	
	$return = array();
	$conn = new mysqli(DB_SERVER, DB_USER, DB_PASSWD, DB_DATABASE);
	$SQL= "SELECT hei_code FROM `nr_programmes` WHERE id= ".$prog_id;
...
htmlRow;
	...
	function checkUserLogin($user, $pass){
		$limit = 5; 
		$SQL = "SELECT orig_password, login_number FROM `users` WHERE user_id = :user AND orig_password = PASSWORD2(:pass);";
	...
	function getInstProgressDetails($filter=array(), $template){
		...
		$SQL = "SELECT nr_programmes.id, nr_programmes.hei_code, nr_programmes.hei_name, nr_programmes.nr_programme_abbr, nr_programmes.nr_programme_name, nr_programmes.heqsf_reference_no, nr_programmes.date_submitted, nr_programmes.ser_doc, nr_programmes.nr_national_review_id, nr_programmes.nr_programme_name
		FROM
			nr_programmes
		WHERE
	...
	function getRecommDetails($lkpField, $lkpValue){
		...
		while($row = $RS->fetch()){
			...
			$recomm['accessDates'] = ($row['recommendation_start_date'] == "1001-01-01" || $row['recommendation_end_date'] == "1001-01-01" ) ? "Not assigned" : $row['recommendation_start_date'] . " to " . $row['recommendation_end_date'];
			$recomm['due-date'] = ($row['recommendation_report_due_date'] == "1001-01-01") ? "Not assigned" : $row['recommendation_report_due_date'];
		}
		return $recomm;
	}	
	...
	function getPanelDetails($lkpField, $lkpValue, $panelStyle = ""){
		...
		$siteVisitSubmittedByAdmin_ind = $this->db->getValueFromTable("nr_programmes","id",$lkpValue,"siteVisitSubmittedByAdmin_ind");	
			...
			$panel['accessDates'] = ($row['panel_start_date'] == "1001-01-01" || $row['panel_end_date'] == "1001-01-01" ) ? "Not assigned" : $row['panel_start_date'] . " to " . $row['panel_end_date'];
			$panel['site_visit_date'] = (($row['site_visit_date'] == "1001-01-01") ? "Not assigned" : $row['site_visit_date']);
		}
		return $panel;		
	}
	...
	function getPrelimAnalysisDetails($lkpField, $lkpValue){
		...
		while($row = $RS->fetch()){
			...
			$prelim['accessDates'] = ($row['analyst_start_date'] == "1001-01-01" || $row['analyst_end_date'] == "1001-01-01" ) ? "Not assigned" : $row['analyst_start_date'] . " to " . $row['analyst_end_date'];
		}
		return $prelim;
	}
	...
	function getScreeningDetails($lkpField, $lkpValue,$form =""){
		...
		if(!empty($screening)){
			foreach($screening as $screen){
				$return .= $screen['screener'] . ', ' . (($screen['date_screening_signed'] != '1001-01-01') ? $screen['date_screening_signed'] : 'In progress') . ', ' . ((!empty($screen['link_report']) ? $screen['link_report'] : 'No report')) . ' <hr>';
			}
		}
...
DROPDOWN;
	...
	function changeProcessAndUser($new_proc, $new_user, $mail_subject="",$mail_content="", $flow=94,$newWorkFlow=""){
...
//print_r($SQL);exit;
		...
		while($row = $RS->fetch()){
			...
			$return['table'] .= '<td>' . $row['sec_group_ref'] . '</td>';
...
//echo 'this is savepanelusers';
//echo $valuesToSave . ' <br> ';
//echo $lkpValue . ' <br> '; 
//print_r($lkpValue);
...
//echo $sqlAvailable;
	...
	function justdeleteGroupProcess($groupId){
		$sql = "DELETE FROM lnk_SecGroup_process WHERE secGroup_ref = :groupId";
		$rs = $this->db->query($sql, compact('groupId'));
	}

	function getScreeningIdForDesktopEvalution($progId){
		$sql = "select s.screening_id from screening s, nr_programmes n 
where s.programme_ref = n.id and n.id=$progId order by s.date_screening_signed desc limit 1;";
...
/*		$table = $tableName > '' ? $tableName : 'rg_meeting_programmes_assignment';
	...
	function saveImprovementPlanAdditionalDocs($progID, $docID, $fieldName) {
		if ($progID > "" &&  $docID > "" && $fieldName > "") {
		  echo $insertSQL = "INSERT INTO nr_programmes (id, " . $fieldName .") VALUES (" . $progID . ", " . $docID . ");";
		  $rsInsert = (!empty($insertSQL)) ? $this->db->query($insertSQL, array()) : '';
		  return true;
		}
		return false;
...
/*function saveImprovementPlanAdditionalDocs($progID, $docID, $fieldName) {
		if ($progID > "" &&  $docID > "" && $fieldName > "") {
		  echo $insertSQL = `UPDATE nr_programmes SET " . $fieldName ." = " . $docID . " WHERE id = " . $progID . ";`;
		  $rsInsert = (!empty($insertSQL)) ? $this->db->query($insertSQL, array()) : '';
		  return true;
		}
		return false;
