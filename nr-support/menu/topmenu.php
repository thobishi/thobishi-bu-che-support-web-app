...
<?php
	...
	array_push($page->scriptFile, "https://ajax.googleapis.com/ajax/libs/jquery/1.10.1/jquery.min.js");
			...
			$SQL = "SELECT 
						distinct(processes_id), menu_perant, menu_sequence_number, processes_desc  
					FROM
		...
		foreach ($subs as $val){
			$numMenu++;
			$SQL = "SELECT 
						distinct(processes_id), processes_desc, menu_alt_name, on_item_click, menu_perant, menu_sequence_number  
					FROM 
						processes, lnk_SecGroup_process  
					WHERE 
						menu_perant = :parent
