...
<div class="hero-unit">
	<h3>RC Meeting</h3>
	<p>
		Reference Committee (RC) meeting date:  <?php $this->showField("rg_meeting_start_date"); ?> to: <?php $this->showField("rg_meeting_end_date"); ?>
	</p>
	<p>
		RC members will have access to the information from: <?php $this->showField("rgc_access_start_date");?> to: <?php $this->showField("rgc_access_end_date"); ?>
		<br /><span class="infoSmall"> (According to programmes and RC Members specified below)</span>
	</p>
</div>

<div class="hero-unit">
<?php 
	$nrc_groupId = $this->getGroupId('RC member');

	echo '<div class = "row-fluid">';
	$this->manageProgrammeDropdown('', 'Programmes','selected', 'rg_meeting');
	$this->manageProgrammeDropdown('WHERE nr_national_reviews.start_date <= CURDATE() AND nr_national_reviews.end_date >= CURDATE() ', '(Click to add programmes)','available', 'rg_meeting');	
	...
	echo '<div class = "row-fluid">';
	$this->manageSecGroupSelect($nrc_groupId, "RC Members", 'selectedMember', 'rg_meeting');
	$this->manageSecGroupSelect($nrc_groupId, "(Click to add members)", 'availableMember' , 'rg_meeting');
	echo '</div>';
	
?>
</div>
<?php
	$this->makeLink("rgc_meeting_minutes_doc", "Upload meeting minutes document");
?>
<script>
	$(".date").each(function(){
	...
	$("#FLD_rg_meeting_start_date").datepicker({
		format: 'yyyy-mm-dd',
		autoclose: true
	});
	
	$("#FLD_rg_meeting_end_date").datepicker({
		format: 'yyyy-mm-dd',
		autoclose: true
	});
	
	$("#FLD_rgc_access_start_date").datepicker({
		format: 'yyyy-mm-dd',
		autoclose: true
	});
	$("#FLD_rgc_access_end_date").datepicker({
		format: 'yyyy-mm-dd',
		autoclose: true
	});	
		
    $('#FLD_rg_meeting_end_date').datepicker().on('changeDate', function(e){
		clearValues = new Array();
		clearValues.push($('#FLD_rg_meeting_start_date'));
		clearValues.push($('#FLD_rg_meeting_end_date'));
		smallerThan($(this).val(), $("#FLD_rg_meeting_start_date").val(), clearValues);
    });
	
    $('#FLD_rgc_access_end_date').datepicker().on('changeDate', function(e){
		clearValues = new Array();
		clearValues.push($('#FLD_rgc_access_start_date'));
		clearValues.push($('#FLD_rgc_access_end_date'));
		smallerThan($(this).val(), $("#FLD_rgc_access_start_date").val(), clearValues);
    });
	
    $('#FLD_rg_meeting_start_date').datepicker().on('changeDate', function(e){
		clearValues = new Array();
		clearValues.push($('#FLD_rg_meeting_start_date'));
		clearValues.push($('#FLD_rg_meeting_end_date'));
		biggerThan($(this).val(), $("#FLD_rg_meeting_end_date").val(), clearValues);
    });
	
    $('#FLD_rgc_access_start_date').datepicker().on('changeDate', function(e){
		clearValues = new Array();
		clearValues.push($('#FLD_rgc_access_start_date'));
		clearValues.push($('#FLD_rgc_access_end_date'));
		biggerThan($(this).val(), $("#FLD_rgc_access_end_date").val(), clearValues);
    });
