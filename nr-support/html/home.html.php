...
<?php
	...
	'41' => array(
			'processName' => 'National Standard Alignment',
			'emptyMessage' => 'There are currently no activities assigned to you.'
	)
    );


/*$nationalStandardGroup = array('41' => array(
			'processName' => 'National Standard Alignment Report',
			'emptyMessage' => 'There are currently1 no activities assigned to you.'
		)      
    ); */
...
function displayArray($array, $indentation = '') {
    foreach ($array as $key => $value) {
        if (is_array($value)) {
            echo $indentation . $key . " => <br>";
            displayArray($value, $indentation . '&nbsp;&nbsp;&nbsp;&nbsp;');
        } else {
            echo $indentation . $key . " => " . $value . "<br>";
        }
    }
}
	
	$additionalProcesses = $this->getMenuProcesses('menu_perant', array('-3', '16', '18', '19', '23', '29','32', '40', '41'));
	
	$processesSorted = array();
	
	$processes = array_merge($processes, $additionalProcesses); //var_dump($processes);
	...
	if(!empty($processesSorted)){
		...
		if(!$noInboxView){ var_dump($processRef);
