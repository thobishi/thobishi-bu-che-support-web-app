...
function valNumberWarning(fld,msg){
	if (fld.value == 0 || isNaN(fld.value)) {
		ans = confirm(msg);
		if (ans){
			return true;
...
function delConsultant(val,name){

	if (confirm("Are you sure that you would like to delete "+ name)) {
		document.defaultFrm.DELETE_RECORD.value = 'd_consultants|consultant_id|'+val;
		moveto('stay');
	}

}

function delContract(val,name){

	if (confirm("Are you sure that you would like to delete "+ name + " contract.")) {
		document.defaultFrm.DELETE_RECORD.value = 'd_consultant_agreements|agreement_id|'+val;
		moveto('stay');
	}

}

function delComment(val,name){

	if (confirm("Are you sure that you would like to delete the displayed comment. Click Cancel to return to the comment.  Click OK to delete the comment and continue.")) {
		document.defaultFrm.DELETE_RECORD.value = 'd_agreement_comments|agreement_comment_id|'+val;
		moveto('next');
	}
