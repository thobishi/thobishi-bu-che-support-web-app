<?php
	...
	$this->formFields['nameSearch']->fieldValue = readPOST('nameSearch');
	$this->showField('nameSearch');
	$this->formFields['typeSearch']->fieldValue = readPOST('typeSearch');
	$this->showField('typeSearch');
	$this->formFields['activeSearch']->fieldValue = readPOST('activeSearch');
...
//echo "Consultants Agreements: " . $agreement_id . "<br>";
	...
	$cons_id = $this->getValueFromTable("d_consultant_agreements", "agreement_id", $agreement_id, "consultant_ref");
...
?>
				<tr>
					<td class="oncoloursoft" align="right" colspan="4">
						<a href="javascript:newAgreementDoc('NEW');moveto('_uploadAgreementDocument_ind');">>> Add new document</a>
					</td>
				</tr>
<?php 
				}
...
<?php 
			...
			if (mysqli_num_rows($rs) > 0) {
				while ($row = mysqli_fetch_array($rs)) {
					...
					$editUploadAgr = ($isOverview && !$isAdministrator && !$isManager) ? " ":'<a href="javascript:newAgreementDoc('.$doc_id.');moveto(\'_uploadAgreementDocument_ind\');">Edit</a>';
					$docHtml .=<<< TEXT
						<tr>
			...
			?>
				...
				<tr>
					<td class="oncoloursoft" align="right" colspan="5">
						<a href="javascript:newAgreementComment('NEW');moveto('_addAgreementComment_ind');">>> Add new comment</a>
					</td>
				</tr>
			<?php 	} ?>
				<tr>
					<td class="ongreycolumnheader">Comment Excerpt</td>
					<td class="ongreycolumnheader">Date comment<br> added</td>
					<td class="ongreycolumnheader">Commenter</td>
					...
					while ($row = mysqli_fetch_array($rs)) {
						...
						if ($isAdministrator){
							$delete_comment = '<td class="ongreycolumn">' .
								'<a href="javascript:newAgreementComment('.$comment_id.');moveto(\'_addAgreementComment_ind\');">Delete</a>'.
								'</td>';
						}				
						$editComment = ($isOverview && !$isAdministrator && !$isManager) ?"": '<a href="javascript:newAgreementComment('.$comment_id.');moveto(\'_addAgreementComment_ind\');">Edit</a>';
						$html =<<< TEXT
							<tr>
								<td class="ongreycolumn">$comment</td>
								<td class="ongreycolumn">$date_added</td>
								<td class="ongreycolumn">$commenter</td>
								<td class="ongreycolumn">$editComment</td>
								$delete_comment
							</tr>
TEXT;
						echo $html;
	...
	<tr>
		...
		<td class="oncolourcolumn">
			...
			?>
				<tr>
					<td class="oncoloursoft" align="right" colspan="6">
						<a href="javascript:newAgreementPerformance('NEW');moveto('_rateContractPerformance_ind');">>> Rate performance</a>
					</td>
				</tr>
			<?php 	}?>
				<tr>
				...
				if (mysqli_num_rows($rs) > 0) {
					while ($row = mysqli_fetch_array($rs)) {
						...
						$editPerformance = ($isOverview && !$isAdministrator && !$isManager) ? " ":'<a href="javascript:newAgreementPerformance('.$rate_id.');moveto(\'_rateContractPerformance_ind\');">Edit</a>';
