...
<?
	...
	$budget_year_id = (isset($_POST['budget_year']) && $_POST['budget_year'] != "") ? $_POST['budget_year'] : $this->getValueFromTable("lkp_budget_year", "lkp_budget_year", $curr_budget_year, "lkp_budget_year_id");
	...
	$this->formFields["budget_year"]->fieldValue = $budget_year_id;
...
{
	$sqlP = $this->getProjectList($project_source,$sec["filter"],$budget_year);
...
<?
		...
		while($rowP = mysqli_fetch_array($rsP)){
			...
			$planned_start_date = "";
			$planned_end_date = "";
			$project_id = ($project_source == 2) ? $rowP["project_ref"] : $rowP["project_id"];
			// get Project Detail
			$sql = <<<SQL
				SELECT *
				FROM `project_detail`
...
SQL;
			...
			if (mysqli_num_rows($rs) > 0){
				...
				$registered = "";
				$budget = $this->getBudget($budget_year,$prj_row["project_id"]);
...
?>
				...
				<tr>
				<td colspan="2" class="oncolourb">Programme: <?php echo echo $dir_description; ?></td>
				<td align="center" colspan="4" class="oncolourb">Budget</td>
				<td colspan="5" align="center" class="oncolourb">Expenditure</td>
				</tr>

				<tr>
				<td align=center class="oncolourb">No</td>
				<td align=center class="oncolourb">Item</td>
				<td align=center class="oncolourb">Planned</td>
				<td align=center class="oncolourb">%<br>Spent</td>
				<td align=center class="oncolourb">Revised</td>
				<td align=center class="oncolourb">%<br>Spent</td>
...
<?
			...
			if ($registered == ""){
				...
				echo '<td align="right">'.$revised_budget."</td>";
				echo '<td align="right">'.$rSpent."</td>";
