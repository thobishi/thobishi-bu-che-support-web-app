...
foreach $qstrings (@qstring) {
	($name, $value) = split (/=/, $qstrings);
	$in{$name} = $value;
}
...
while (read (STDIN ,$LINE, 4096) && $bRead < $len )
{
	...
	select(undef, undef, undef,0.25);	# sleep for 0.35 of a second.
...
my $url = $php_uploader . "?sid=$sessionid&cmd=$in{'cmd'}&val=$in{'val'}&field=$in{'field'}";
