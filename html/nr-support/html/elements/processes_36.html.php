		...
		foreach($processes as $processCount => $process){
			...
			if(!empty($programmeData)){
				foreach($programmeData as $programme){
					...
					$heqcRecommendationReportLink = $this->createDocLink($programme['heqc_recommendation_report_doc'], 'RC Report');
					...
					$heqc_nrc_reportLink = $this->createDocLink($programme['heqc_nrc_report_doc'], 'NRC Report');
					$heqc_nrc_report = (!empty($heqc_nrc_reportLink)) ? $heqc_nrc_reportLink :  "Not uploaded";
					$processData[$processCount]['heqc_nrc_report_doc'] = $heqc_nrc_report;	
		...
		<?php
			if(!empty($processData)){
				foreach($processData as $processInfo){
					...
					echo '<td>' . $processInfo['heqc_nrc_report_doc'] . '</td>';						
					echo '</tr>';
				}
			}
			else{
