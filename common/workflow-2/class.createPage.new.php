...
class createPage extends security {
	...
	function initTags() {
		//$this->htmlStart = "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">\n<html>\n";
		$this->htmlStart = "<html>\n";
		$this->title = "";
	...
	function readTemplate() {
		$this->verifySecurity ();
		if ( Settings::isIsset('template') ) {
			if (Settings::get('template') > "") {
				$tmplName = Settings::get('relativePath')."templates/".Settings::get('template').".template.php";
...
//		$template_id = 0;
		...
		$SQL = "SELECT * FROM `work_flows` WHERE template='".Settings::get('template')."'";
		$RS = mysqli_query($SQL);
...
//			$template_id = $row["template_info_id"];
		...
		*/
		$SQL = "SELECT * FROM `template_action` WHERE template_name='".Settings::get('template')."' ORDER BY sec_no";
		$RS = mysqli_query($SQL);
		$action_obj = array();
		...
		*/
		...
		$SQL = "SELECT * FROM `template_field` WHERE template_name='".Settings::get('template')."'";
		$RS = mysqli_query($SQL);
		while ($row = mysqli_fetch_array($RS)) {
...
//			$this->formFields[$row["fieldName"]]->fieldValuesArray = explode("|", ($row["fieldValuesArray"]));
	...
	function createHTML($file) {
		if ($file > "") {
			...
			$codePage = Settings::get('relativePath')."proc/".$file.".pre.php";
			if (file_exists($codePage)) {
				include ($codePage);
			}
			include (Settings::get('relativePath')."html/".$file.".html.php");
		}
	}
