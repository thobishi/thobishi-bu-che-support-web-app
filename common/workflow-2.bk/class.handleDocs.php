...
class handleDocs extends pageForm {
	...
	*/
	function makeDocInput($obj,$DBfld){
		...
		echo '<INPUT  TYPE="hidden" class="'.$obj->fieldClass.'" NAME="'.$DBfld.$obj->fieldName.'" VALUE="'.$obj->fieldValue.'" '. $dataHolder .'>';
	}
	...
	*/
	...
	function makeLink($field,$text="", $table="", $keyFLD="", $keyVal="",$DateUploadedField=""){
		...
		if ($rs && ($rs->rowCount() > 0)){
			...
			if ($doc->isDoc()){
				if (! $this->view ) {
					...
					if($DateUploadedField > "") {
						$this->db->setValueInTable($table, $keyFLD, $keyVal, $DateUploadedField, $doc->getDateUpdated());
					}
					echo '<table class="table table-hover table-bordered table-striped">';
					echo '<tr><td colspan="2" class="columnHeading rowHeading">' . $text . '</td></tr>';
					echo "<tr>";
					echo "<td class='columnHeading'>File: </td>";
					echo "<td width='60%'><a href='".$doc->url()."' target='_blank'>".$doc->getFilename()."</a></td>";
					echo "</tr>";
					echo "<td class='columnHeading'>First Uploaded: </td>";
					echo "<td>".$doc->getDateCreated()."</td>";
					echo "</tr>";
					echo "<tr>";
					echo "<td class='columnHeading'>Last Uploaded: </td>";
					echo "<td>".$doc->getDateUpdated()."</td>";
					echo "</tr>";
					echo "<tr>";
					echo "<td class='columnHeading'>Replace Uploaded File: </td>";
					...
					echo "</tr><tr>";
					echo "<td class='columnHeading'>Delete File: </td>";
					echo "<td><a class=\"deleteFile\" data-field=\"FLD_" . $field . "\" data-delete_record=\"documents|document_id|" . $doc->getDocID() . "\" data-moveto=\"stay\" href=\"#\">Click here to delete the uploaded file.</a></td>";
					echo "</tr>";
					echo "</table>";
				}else {
			...
			//we have a new record, with no documents linked to it yet
				if (! $this->view ) {
					$this->showField($field);
					if($DateUploadedField > "") {
						$this->db->setValueInTable($table, $keyFLD, $keyVal, $DateUploadedField, "DEFAULT");
					}
					echo '<table class="table table-hover table-bordered table-striped">';
					echo '<tr><td colspan="2" class="columnHeading rowHeading">' . $text . '</td></tr>';
					echo "<tr>";
					...
					echo "</tr>";
					echo "<tr>";
					echo "<td class='columnHeading'>Date Created: </td>";
					echo "<td>N/A</td>";
					echo "</tr>";
					echo "<tr>";
					echo "<td class='columnHeading'>Last Updated: </td>";
					echo "<td>N/A</td>";
					echo "</tr>";
					echo "<tr>";
					echo "<td class='columnHeading'>Upload File: </td>";
	...
	function makeLinkWithoutDelete($field,$text="", $table="", $keyFLD="", $keyVal="",$DateUploadedField=""){
		$table = ($table > "")?($table):($this->dbTableCurrent);
		$keyFLD = ($keyFLD > "")?($keyFLD):($this->dbTableInfoArray[$this->dbTableCurrent]->dbTableKeyField);
		$keyVal = ($keyVal > "")?($keyVal):($this->dbTableInfoArray[$this->dbTableCurrent]->dbTableCurrentID);
		$SQL = "SELECT ".$field." FROM ".$table." WHERE ".$keyFLD." = '".$keyVal."'";
...
//echo $SQL;
	...
	*/
	function generateReport($callBack="",$extention=".doc"){
		...
		$tmpFile = tempnam(Settings::get('TmpDir'),"");
	...
	*/
	function makeSinglePaperEval($id){
		...
		$SQL2 = "SELECT application_comp_all FROM application_summery_comments WHERE application_ref = ".$id;
	...
	*/
	function makePaperEvalReport($id){
		$content = "";
		$SQL = "SELECT application_id FROM Institutions_application WHERE application_status=1 and AC_Meeting_ref=".$id;
		$rs = mysqli_query($SQL);
...
/**********************************************************************************/
	...
	function perQuestion2($table,$display1,$display2,$key,$ques,$id,$text){
		$tmp ="";
		$SQL = "SELECT ".$display2." FROM ".$table." WHERE ".$key."=".$id;
		$rs = mysqli_query($SQL);
...
//			$tmp .= $ques.". ".$text.":<br><br>";
	...
	function perEvaluatoionProgOutcomeRecommend ($table,$display1,$key,$id,$text) {
		$tmp ="";
		$SQL = "SELECT ".$display1." FROM ".$table." WHERE ".$key."=".$id;
		$rs = mysqli_query($SQL);
		if (mysqli_num_rows($rs) > 0){
			$row = mysqli_fetch_array($rs);
			$tmp .= "<b>".$text."</b>:<br>";
			$tmp .= (($display1 == "AC_desision_recommend")?($this->db->getValueFromTable("lkp_desicion", "lkp_id", $row[$display1], "lkp_title")):($row[$display1]))."<br><br>";
		}
		return $tmp;
	}
...
/**********************************************************************************/
	...
	*/
	function perQuestion($table,$display1,$display2,$key,$ques,$id,$text){
		$tmp ="";
		$SQL = "SELECT ".$display1.",".$display2." FROM ".$table." WHERE ".$key."=".$id;
		$rs = mysqli_query($SQL);
		if (mysqli_num_rows($rs) > 0){
			$row = mysqli_fetch_array($rs);
	...
	*/
	function makeTop($id){
		$content = "";
		$SQL = "SELECT lnk_priv_publ_desc,HEI_name,CHE_reference_code,program_name,NQF_ref FROM Institutions_application,HEInstitution,lnk_priv_publ  WHERE lnk_priv_publ_id=priv_publ and  HEI_id=institution_id and application_id = ".$id;
		$rs = mysqli_query($SQL);
		if (mysqli_num_rows($rs) > 0){
			$row = mysqli_fetch_array($rs);
	...
	*/
	function genSiteVisitReportPerApplication ($application_ref) {
		$html = "";
		$SQL = "SELECT site_ref, site_visit, object_sitevisit_visit FROM `siteVisit` WHERE application_ref=".$application_ref;
		$RS = mysqli_query($SQL);
		while ($row = mysqli_fetch_object($RS)) {
			if ($row->site_visit == 'Yes') {
				...
				$html .= '<td>No Site Visit for site: '.$this->db->getValueFromTable("institutional_profile_sites", "institutional_profile_sites_id", $row->site_ref, "location").'</td>';
				$html .= '</tr><tr>';
	...
	*/
	function genSiteVisitReport ($application_ref="", $site_ref="") {
		...
		$SQL = "SELECT * FROM `evalSiteVisitReport` WHERE application_ref='".$application_ref."'";
		$RS = mysqli_query($SQL);
...
html;
		...
		$html .=  '<table border="1"><tr>';
		$html .=  '<td valign="top" colspan="'.($TDcount).'">'.$this->db->getValueFromTable("institutional_profile_sites","institutional_profile_sites_id", $site_ref,"location").'</td>';
		$html .=  '</tr><tr>';
		while ($row = mysqli_fetch_object($RS)) {
		...
		while ($row = mysqli_fetch_object($RS)) {
			...
			$SQLrow = "SELECT commend, documentation, comments FROM `siteVisit_report` WHERE ".$_site." application_ref='".$application_ref."' AND siteVisit_report_areas_ref='".$row->siteVisit_report_areas_id."'";
...
//				if (($row->main_heading=="OTHER") && ($count==11)) $html .= '<td valign="top" rowspan="1">'.$ROWrow->comments.'</td>';
	...
	*/
	function makeACAgenda($ddate,$id){
		...
		$SQL = "SELECT HEI_name,program_name,NQF_ref,CHE_reference_code FROM Institutions_application,HEInstitution WHERE HEI_id=institution_id and application_status=1 and AC_Meeting_ref=".$id;
		$rs = mysqli_query($SQL);
		while ($row = mysqli_fetch_array($rs)){
