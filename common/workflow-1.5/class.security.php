...
class security extends dbFunctions {
		...
		if ($pass > "") {
			...
			if ($row = mysqli_fetch_array ($rs)) {
				if ($row["active"] > 0) {
					...
					$this->noLogonMessage = "<span class=\"expiry\">Access to the accreditation system is not granted.</span> <a href='javascript:showHide(document.all.err);'>Possible reasons</a><br><div id=\"err\" style=\"display:none\"><table align='center'><tr><td align='left'><span class=\"special\"><ul><li type='1'>You have not registered a user profile as yet, please apply for a login name.</li><li type='1'>You have entered an incorrect username or password, please try again.</li><li type='1'>You have applied for a profile, but your application has not been processed yet.</li></ul></span></td></tr></table></div>";
					return 0;
				}
			}
		}else {
	...
	*/	
	function updateUserLastLoginDate($id=0) {
		mysqli_query ("UPDATE `users` SET login_number=(login_number+1), last_login_date='".date("Y-m-d")."' WHERE user_id=".$id);
	}
