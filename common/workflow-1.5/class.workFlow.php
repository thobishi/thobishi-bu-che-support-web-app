...
class workFlow extends flowLogic {
	...
	var $dbConn;
	var $AP;	// class activeProcess
	var $PS;	// class pageSecurity

	function workFlow ($flowID) {
		$this->dbConn = dbConnect::createConnection ();
		$this->AP = new activeProcess ($this->dbConn);
		$this->PS = new pageSecurity ($this->dbConn);
...
/*	function runPostScript() {
	...
	// Only called from index.php if ID in GET
	function setActiveWorkFlow ($id) {

			if ($this->AP->setActiveProcessID ($id, $this->currentUserID)) {
			$row = $this->AP->getData ();
	...
	function parseOtherWorkFlowProcess ($activeFlow) {
		...
		$this->AP->setActiveProcessID($activeFlow);  // BUG need userREF
		if ($row = $this->AP->getData ()) {
			$arr = split("&", $row['workflow_settings']);
			foreach ($arr as $a) {
				$parts = split("=", $a);
				if (! strncmp($parts[0], "DBINF_", 6) ) {
					$this->db_settingsKey = $parts[0];
					$p = split("___", substr ($parts[0], 6));
	...
	*/
	function makeWorkFlowStringFromCurrent ($curtable, $keyfield, $keyvalue) {
		...
		// Chnage the copy of org WorkFlow
			...
			if (! strncmp($key, "GRID_", 5) ) {
				...
				if (! (stristr($key, "save")) ) {
					...
					if ($grid_save_flag) {
						...
						foreach ($save_grid_fields_array AS $grid_save_val) {
							$field_val = explode("|", $grid_save_val);
							$this->setValueInTable($field_val[0], $field_val[1], $field_val[2], $field_val[3], $field_val[4]);
						}
					}
		...
		if (count ($saveFields) > 0) {
			if ( isset($this->dbTableCurrent) && isset($this->dbTableInfoArray[$this->dbTableCurrent]) ) {
				if ( $this->dbTableInfoArray[$this->dbTableCurrent]->dbTableCurrentID != "NEW" ) {
					...
					$this->writeLogInfo(10, $logType, $SQL."  --> ".mysqli_error(), $errorMail);
					$this->writeLogInfo(100, "POST DATA", var_export($_POST, true), $errorMail);
				}
			}
		}
	...
	function writeAuditTrail($id,$subject,$descr){
		if ($id != 0) {
			if (! ($prc_ref = $this->AP->getProcess ($id)) ) {
				$prc_ref = $this->flowID;
			}
			...
			if ($this->audit_level >= $aud_lev) {
				...
				$SQL = <<<sqlInsert
					...
					(`active_processes_ref`, `application_ref`, `institution_ref`,`user_ref`, `session_id`, `ip_number`,
					`process_ref`,`process_desc`,`work_flows_ref`,`audit_subject`,`audit_text`,`audit_level`,
					`date_updated`,`workflow_settings`)
					VALUES ('$id', '$app_ref', '$ins_ref', '$usr_ref','$ses_id','$REMOTE_ADDR',
	...
	*/
	function writeLogInfo($level, $subject, $log_var, $mail=false) {
		if ($this->log_level >= $level) {
			$proc = $this->AP->getProcess ($this->active_processes_id);

			$log_var = system_escape ($log_var);
			$SQL = "INSERT INTO `workflow_log_file` VALUES (NULL, '".$level."','".$this->currentUserID."', '".system_escape($_SERVER['REMOTE_ADDR'])."', '".$proc."', NOW(), '".$this->template."', '".$subject."'";
			$SQL .= ", ";
			...
			if ($mail) {
				$this->misMailByName("heqc@octoplus.co.za", "(".CONFIG.") error report", "ID: ".mysqli_insert_id ()."\n".$log_var, "", "HEQC error log");
			}
		}
	}
...
//		}
		...
		*/
		if ((isset($_POST["VALIDATION"])) && ($_POST["VALIDATION"] > "")) {
			if (($_POST["MOVETO"] != "next") && ($_POST["MOVETO"] != "previous")) {
	...
	function checkChangeToRecord () {
		if (isset($_POST["CHANGE_TO_RECORD"]) && ($_POST["CHANGE_TO_RECORD"] > "")) {
			$change = explode ("|", $_POST["CHANGE_TO_RECORD"]);
			switch (count ($change)) {
				case 1:
	...
	function checkDeleteRecord() {
		if (isset($_POST["DELETE_RECORD"]) && ($_POST["DELETE_RECORD"] > "")) {
			$delete = system_escape (explode ("|", $_POST["DELETE_RECORD"]));
			$del_SQL = "DELETE FROM ".$delete[0]." WHERE ".$delete[1]."='".$delete[2]."'";
			$errorMail = false;
			mysqli_query($del_SQL) or $errorMail = true;
...
*/
	...
	function updateActiveProcesses () {
		if (! ($this->viewPage>0) ) {
			if (isset($this->currentUserID) && (isset($this->active_processes_id)) && ($this->currentUserID > 0) && ($this->flowID != 1)) {
				$this->AP->setActiveProcessID ($this->active_processes_id);
				if ($this->doChangeDueDate) {
					$this->doChangeDueDate = false;
					$this->AP->setDueDate ($this->getDueDate($this->flowID));
					$this->AP->setExpiryDate ($this->getExpiryDate($this->flowID));
				}
				...
				$this->AP->setProcess ($this->flowID);
				$this->AP->setWorkflow ($this->flowID);
				$this->AP->setWorkflowSettings (system_escape($this->getStringWorkFlowSettings ()));
				$this->AP->setUser ($this->currentUserID);
				$this->AP->update ();
	...
	function completeActiveProcesses () {
		if (isset($this->currentUserID) && (isset($this->active_processes_id)) && ($this->currentUserID > 0) && ($this->flowID > 2) && ($this->active_processes_id>0)) {
			...
			$this->AP->setActiveProcessID ($this->active_processes_id);
			if (!$this->AP->setCompleted ()) {
				$this->writeLogInfo(1000, "ActiveProcess", "Active Processes CAN NOT be completed:\n\nID:".$this->active_processes_id);
			}
	...
	function changeActiveProcesses ($process, $user, $work_flow=0) {
		if (isset($this->currentUserID) && (isset($this->active_processes_id)) && ($this->currentUserID > 0) && ($this->active_processes_id>0)) {
			...
			$SQL = "UPDATE active_processes SET".
				...
				", work_flow_ref = ".$work_flow.
				", workflow_settings = \"".system_escape($this->getStringWorkFlowSettings ()).'"'.
				", user_ref = ".$user.
				", last_updated = now()".
				$dueDate.
				$expiryDate.
	...
	*/
	...
	function misMailByName ($email, $subject, $message, $cc="", $ownFromAdr=true, $additionalHeaders="", $isHTML=false) {
		$mail = new PHPMailer();
...
*/		$mail->WordWrap = 75;
	...
	function validateFields ($fieldTemplate){
...
//changed $this->dbTableCurrent to $tblRow[0]: 2004-08-20
	...
	// short for function table_field_info to use current Process ID
...
//		}
		...
		$progDisc = $this->table_field_info($active_processes_id, "ProgramName");
	...
	function checkUserInDatabase($email, $surname, $name) {
		$user_id = 0;
		...
		*/
			...
			$insertSQL = "INSERT INTO `users` (user_id, name, surname, password, email, active, institution_ref, institution_name) VALUES (NULL, '".$name."', '".$surname."', '', '".$email."', 1, 0, 'Evaluator')";
			...
			$this->misMail($user_id, "You are registered with the CHE accreditation system", $message);
		}

		// make sure $user_id is the existing or new id depending on what happened ontop.
	...
	*/
	function mimemail ($to, $from, $subject, $message, $filelist="") {
		...
		$this->misMailByName ($to, $subject, $email_message, "", true, $addHeaders);
