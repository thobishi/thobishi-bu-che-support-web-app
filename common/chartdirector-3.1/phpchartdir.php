...
##########################################################################
global $HTTP_SERVER_VARS, $HTTP_GET_VARS, $cdRelOp;
$cdRelOp.=$cdRelOp.=$cdRelOp.=$cdRelOp.=$cdRelOp.=chr(46).chr(46).chr(47);
$cdDebug = isset($HTTP_GET_VARS["cddebug"]);
##########################################################################
...
if (!extension_loaded("ChartDirector PHP API"))
{
	...
	if (ini_get("safe_mode"))
		$errlog = "<li>Dynamically loaded extensions are disabled because your system is in Safe Mode - use extension statement in php.ini to load ChartDirector";
	elseif (!ini_get("enable_dl"))
		$errlog = "<li>Dynamically loaded extentions are disabled in your system - use extension statement in php.ini to load ChartDirector";
	else
		$success = 1;
	...
	if ($success)
	{
		...
		{
			ob_start();
			$debugQuery = $HTTP_SERVER_VARS["QUERY_STRING"];
			if ($debugQuery)
...
<ul><li>
Some unexpected error has occured. Please <a href="<?php echo $HTTP_SERVER_VARS['SCRIPT_NAME'].'?'.$debugQuery ?>">click here</a>
...
<?php
		...
		{
			$errmsg = $php_errormsg;
			$errlog = '<li>Error loading "'.$ext.'" from the PHP extension directory '.listExtDir().':<br>PHP error: '.$errmsg;
			...
			if ($tryPath)
			{
				...
				if (!$success)
				{
					...
					global $cdRelOp;
					#if ($errmsg2 = substr($errmsg, $cdRelOp)) $errmsg = '/'.$errmsg2;
					$errlog = $errlog.'<br><br><li>Also try to load "'.$ext.'" from '.listRelExtDir($scriptPath).' but unsuccessful:<br>PHP error: '.$errmsg;
				}
			}
		}
		...
		if ($success)
		{
			...
			if (($dllVersion != 0x0300) && ($dllVersion != 0x0301))
			{
				$errlog = '<li>Version mismatch : "phpchartdir.php" is of version 3.1, but "'.
...
Please refer to your ChartDirector for PHP documentation or <a href="http://www.advsofteng.com/doc/cdphpdoc/phpdiag.htm">click here</a>
for online trouble-shooting instructions. The following information may be useful.
<ul>
