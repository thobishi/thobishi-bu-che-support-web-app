 ...
 */
	function connect() {
		...
		if (!$config['persistent']) {
			$this->connection = mysqli_connect($config['host'] . ':' . $config['port'], $config['login'], $config['password'], true);
			$config['connect'] = 'mysqli_connect';
		} else {
			$this->connection = mysqli_pconnect($config['host'] . ':' . $config['port'], $config['login'], $config['password']);
		}

		if (mysqli_select_db($config['database'], $this->connection)) {
			$this->connected = true;
		}
 ...
 */
	function value($data, $column = null, $safe = false) {
		...
		switch ($column) {
			...
			default:
				$data = "'" . mysqli_real_escape_string($data, $this->connection) . "'";
			break;
		}
		return $data;
	}
