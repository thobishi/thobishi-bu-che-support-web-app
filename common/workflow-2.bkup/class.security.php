...
if ($conn->connect_errno) {
    ...
    exit();
		...
		if ($pass > "") {
			$SQL = "SELECT * FROM ".$this->userTable." WHERE UPPER(".$this->usernameField.") = UPPER(:user) AND ".$this->passwordField . " = PASSWORD2(:pass)";
		//	$SQL = "SELECT * FROM ".$this->userTable." WHERE UPPER(".$this->usernameField.") = UPPER('$user') AND ".$this->passwordField." = PASSWORD2('$pass')";
		...
		//  print_r($SQL);
			...
			// echo 'row found';exit;
					if ($row["active"] > 0) {
						Settings::set('currentUserID', $row[$this->userIDfield]);
						$this->setUserSession();
						$this->updateUserLastLoginDate(Settings::get('currentUserID'));// read goto or set it to 1 if not available
					
// read goto or set it to 1 if not available
//$id = $this->addActiveProcesses (2, $row[$this->userIDfield], 0);
