...
class flowLogic extends handleDocs {
	...
	function logicCheckLabel($workFlowID, $label, $direction) {
		...
		//if ($label === 'national_standard_alignemnt_report') {
			...
			//$rs = $this->db->query($SQL, compact('hei_id', 'nr_national_review_id'));
			//$row = $rs->fetch();
			////$curTable = $this->db->getValueFromTable("work_flows", "work_flows_id", '265', "template_dbTableName");
			////$curField = $this->db->getValueFromTable("work_flows", "work_flows_id", '265', "template_dbTableKeyField");
			//$desc = $this->updateWorkFlowSettingforIPlan ("nr_programmes", "id", $row['id']);
		  //}
		return ($workFlowID);
	}
