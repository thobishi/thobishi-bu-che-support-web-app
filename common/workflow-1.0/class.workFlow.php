<?php
//require_once ('class.flowLogic.php');
...
//print_r($this->dbTableInfoArray);
	...
	function parseOtherWorkFlowProcess ($activeFlow) {
		...
		$SQL = "SELECT workflow_settings FROM active_processes WHERE active_processes_id = $activeFlow";
		$conn = $this->getDatabaseConnection();
	//	$sm = $conn->prepare($SQL);
...
//					die ($_POST['TMPL_NAME']." ".$fldName." ".$val);
					...
					//$RSfield = mysqli_query ($conn, $SQLfield);
				...
				if (!stristr($key, "save") && !stristr($key, "deleted")) {
					...
					if (!$grid_save_flag) {
						if (count($field_val) > 0) {
							...
							$del_rs = mysqli_query($conn, "DELETE FROM `".$field_val[0]."` WHERE ".$field_val[1]."='".system_escape_i($field_val[2], $conn)."'");
		...
		if (count ($saveFields) > 0) {
			...
			if ( isset($this->dbTableCurrent) && isset($this->dbTableInfoArray[$this->dbTableCurrent]) ) {
				if ( $this->dbTableInfoArray[$this->dbTableCurrent]->dbTableCurrentID != "NEW" ) {
					...
					$SQL = "SELECT write_current_user_to_db FROM processes WHERE processes_id = ".$this->flowID;
					...
					if ($try_sql  && mysqli_query($conn, $SQL)) {
						$logType = "SQL";
						$errorMail = false;
						$this->dbTableInfoArray[$this->dbTableCurrent]->dbTableCurrentID = mysqli_insert_id ($conn);
	...
	function writeAuditTrail($id,$subject,$descr){
		if ($id != 0){
			...
			if ($this->audit_level >= $aud_lev) {
				...
				$ses_id = session_id();
				$REMOTE_ADDR = isset($_SERVER['REMOTE_ADDR']) ? system_escape_i($_SERVER['REMOTE_ADDR'], $this->getDatabaseConnection()) : "";
				$SQL = <<<sqlInsert
					...
					'$aud_date','$wkf_set')
sqlInsert;
				mysqli_query($this->getDatabaseConnection(), $SQL);
			}
	...
	*/
	function writeLogInfo($level, $subject, $log_var, $mail=false) {
		...
		if ($this->log_level >= $level) {
			...
			$REMOTE_ADDR = isset($_SERVER['REMOTE_ADDR']) ? system_escape_i($_SERVER['REMOTE_ADDR'], $conn) : "";
			$SQL = "INSERT INTO `workflow_log_file` VALUES (NULL, '".$level."','".$this->currentUserID."', '".$REMOTE_ADDR."', '".$proc."', NOW(), '".$this->template."', '".$subject."'";
			$SQL .= ", ";
			...
			if ($mail) {
				$this->misMailByName("heqc@octoplus.co.za", "(".CONFIG.") error report", "ID: ".mysqli_insert_id ($this->getDatabaseConnection())."\n".$log_var, "", "HEQC error log");
			}
...
//		}
	...
	function checkDeleteRecord() {
		...
		if (isset($_POST["DELETE_RECORD"]) && ($_POST["DELETE_RECORD"] > "")) {
			...
			$this->writeLogInfo(10, "SQL-DELREC", $del_SQL."  --> ".mysqli_error($conn), $errorMail);
...
//				if ($row["active"] == 0) {
					$del_SQL = "DELETE FROM ".$delete[0]." WHERE ".$delete[1]."='".$delete[2]."'";
					$del_rs = mysqli_query($this->getDatabaseConnection(), $del_SQL);
//				}
			}
		}
*/
	...
	function setMoveToFlow ($to) {
		...
		if (substr($to, 0, 1) == '_') {
			$label = substr($to, 1);
			$SQL = "SELECT * FROM work_flows WHERE processes_ref = ".$this->flowID." AND command = '".$label."' AND workFlowType_ref = 6";
	...
	function setPreviousFlow () {
		...
		if (! $this->readWorkFlowSettings ($SQL, "P")) {
			$SQL = "SELECT * FROM processes WHERE processes_id = ".$this->flowID;
			
			$rs = mysqli_query($this->getDatabaseConnection(), $SQL);
			if ($row = mysqli_fetch_array($rs)) {
				if ($row["may_go_previous"] == "yes") {
					$SQL = "SELECT * FROM processes WHERE currentFlow_next_process_ref = ".$this->flowID;
					$rs = mysqli_query($this->getDatabaseConnection(), $SQL);
					if ($row = mysqli_fetch_array($rs)) {
		...
		if (! ($this->viewPage>0) ) {
			if (isset($this->currentUserID) && (isset($this->active_processes_id)) && ($this->currentUserID > 0) && ($this->flowID != 1)) {
				...
				$SQL = "UPDATE active_processes SET".
					...
					", work_flow_ref = ".$this->workFlowID.
					", workflow_settings = \"".system_escape_i($this->getStringWorkFlowSettings (), $this->getDatabaseConnection()).'"'.
					", user_ref = ".$this->currentUserID.
					", last_updated = now()".
					$dueDate.
					$expiryDate.
					" WHERE active_processes_id = ".$this->active_processes_id;
	...
	function getDueDate($process, $otherDate="", $dateType="processes_due_duration") {
		...
		if ($row = mysqli_fetch_array($rs)) {
			if ($row[$dateType]>0) {
				...
				$ret = date("Y-m-d", time(0, 0, 0, date("m", $theDate)  , date("d", $theDate)+$row[$dateType], date("Y", $theDate)) );
			}
		}
...
//note that if you have added a CC and are testing, you will not see that it is being cc'd
		if (defined('WRK_ALT_EMAIL')) {
			$email = WRK_ALT_EMAIL;
			$cc = '';
		} else {  // optional cc of ALL emails to CHE user for production
			$bcc = $this->getDBsettingsValue("usr_system_emails");
			if ($bcc > '') {
				$bcc_arr = explode(",",$bcc);
				foreach ($bcc_arr as $bc){
					$mail->AddBCC($bc);
				}
			}
		}
...
*/		$mail->WordWrap = 75;
	...
	function validateFields_old ($fieldTemplate){
		...
		while ($row = mysqli_fetch_array($RS)) {
			...
			$regexpRS = mysqli_query($this->getDatabaseConnection(), $regexpSQL);
...
//changed to get info from work_flows instead of template info table
				...
				$tblRS = mysqli_query($this->getDatabaseConnection(), $tblSQL);
...
//changed $this->dbTableCurrent to $tblRow[0]: 2004-08-20
					...
					$valueRS = mysqli_query($this->getDatabaseConnection(), $valueSQL);
					if ($valueRS && ($valueRow = mysqli_fetch_array($valueRS))) {
						...
						if (preg_match($regexpRow[0], $valueRow[0])) {
							...
							$RSdesc = mysqli_query($this->getDatabaseConnection(), $SQLdesc);
							...
							$moveToRS = mysqli_query($this->getDatabaseConnection(), $moveToSQL);
		...
		mysqli_conn($this->getDatabaseConnection());
	}
...
htmlRow;
		...
		if ( isset($tables) && ($row = mysqli_fetch_array ($rs)) ) {
			if (isset ($tables[$row["table_name"]]) ) {
				if (!$desc) {
					...
					$rs2 = mysqli_query($this->getDatabaseConnection(), $SQL);
					if ($row2 = mysqli_fetch_array ($rs2)) {
						...
						if ($desc_field == 'SiteVisitInstitution') $ret = $this->getValueFromTable("HEInstitution", "HEI_id", $row2[0], "HEI_name");
					}
				} else {
	...
	function workflowDescription ($active_processes_id, $proc_id) {
		...
		if ($HEQC_ref> "" && !stristr($ret,"$HEQC_ref")){
			$ret .= " - ".$HEQC_ref;
		}
		$progDisc = $this->table_field_info($active_processes_id, "ProgrammeName");
		if ($progDisc > "" && !stristr($ret,"$progDisc")){
			$ret .= " - ".$progDisc;
		}
...
function checkUserInDatabase($title_ref, $email, $surname, $name, $groupID=0) {
		...
		if ($row = mysqli_fetch_array($RS)) {
			...
			if (!$row["active"]) {
				...
				$activeRS = mysqli_query($this->getDatabaseConnection(), $activeSQL);
			}
		...
		*/
			...
			$insertRS = mysqli_query($this->getDatabaseConnection(), $insertSQL);
			$user_id = mysqli_insert_id($this->getDatabaseConnection());
			...
			$this->misMail($user_id, "You are registered with the HEQC-online system", $message);	
			
			//start

			$SQLGetID = "SELECT * FROM Eval_Auditors,users where  users.email=Eval_Auditors.E_mail ".
			" AND  E_mail='".$email."'";
	...
	//$rs = $sm->get_result();
			...
			$rs = mysqli_query($this->getDatabaseConnection(), $SQLGetID);
			if ($row = mysqli_fetch_array ($rs)) {
				$Getuser_id=$row['user_id'] ;
				$users_id=$row['user_id'] ;
				$useremails=$row['email'] ;
				//echo $user_id;
				//echo $useremails;
				//$activeSQL = "UPDATE `Eval_Auditors` SET user_ref='".$user_id."' WHERE E_mail='".$user_email."'";
							
				$SQLUpdate = "UPDATE `Eval_Auditors` SET user_ref='".$Getuser_id."' WHERE E_mail='".$useremails."'";
				//echo $SQLUpdate;
				$RS = mysqli_query($this->getDatabaseConnection(), $SQLUpdate);
				
				$gSQL = "SELECT * FROM `sec_UserGroups` WHERE sec_user_ref='".$Getuser_id."' AND sec_group_ref = '".$groupID."'";
				$gRS = mysqli_query($this->getDatabaseConnection(), $gSQL);
				//if ((!($g_row = mysqli_fetch_array($gRS))) && ($groupID=0)) {

				//echo $gSQL;

				if (!($g_row = mysqli_fetch_array($gRS))) {
				$insertSQL = "INSERT INTO `sec_UserGroups` (sec_UserGroups_id, sec_user_ref, sec_group_ref) VALUES (NULL, '".$Getuser_id."', '".$groupID."')";
				$insertRS = mysqli_query($this->getDatabaseConnection(), $insertSQL);
			}
			//end
...
*/
	...
	*/
	...
	function wrk_getNextWorkFlow($workFlowID, $direction="N") {
			...
			$SQL = "SELECT * FROM work_flows WHERE processes_ref = ".$row["processes_ref"]." AND sec_no ".$operator." ".$row["sec_no"]." ORDER BY sec_no".$order." LIMIT 0,1";
			$rs = mysqli_query($this->getDatabaseConnection(), $SQL);
			$row = mysqli_fetch_array($rs);
	...
	function cleanTempActiveProccesses () {
		if ($this->currentUserID > 0) {
			$SQL = "SELECT active_processes_id FROM active_processes, processes WHERE processes_ref = processes_id AND keep_workflow_settings = 'temp' AND status = 0 AND user_ref = ".$this->currentUserID." ORDER BY last_updated DESC";
			...
			$SQL = "UPDATE active_processes SET status=1 WHERE processes_ref >= 0 AND processes_ref <= 2 AND status = 0 AND user_ref = ".$this->currentUserID;
			$rs = mysqli_query($this->getDatabaseConnection(), $SQL);
