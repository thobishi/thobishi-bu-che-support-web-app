...
$rs = mysqli_query($conn, $SQLinst);
    ...
    if($Datesubmitted != 'no date'){
        ...
        $usr_setting = ("usr_finance_siteapp_invoice");
       

    }else{
    
        $usr_setting = ("usr_finance_site_invoice");
         
    }
   // $usr_setting = ("usr_siteapp");
   
	$new_user = $this->getValueFromTable("settings", "s_key", $usr_setting, "s_value");
    
  //  $new_user=4839;
  // echo $new_user;
//$this->changeProcessAndUser(225, $new_user, " test", "test");
...
$to = $this->getValueFromTable("users", "user_id", $new_user, "email");
    ...
    $id = $this->addActiveProcesses (225, $new_user);
