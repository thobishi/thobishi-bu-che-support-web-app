...
APPUPD;*/
			...
			$update2SQL = <<<APPUPD
				UPDATE ia_proceedings 
				SET application_status_ref = 1, ac_meeting_ref = 0, lkp_AC_agenda_type_ref = NULL 
				WHERE ia_proceedings.ia_proceedings_id = {$u_row["ia_proceedings_id"]}
APPUPD;
			$errorMail = false;
...
SITEUPD;
			$errorMail = false;
			mysqli_query($updS) or $errorMail = true;
...
// 		2012-04-17 AND a.application_status = 1
	...
	while ($row = mysqli_fetch_array($ars)) {
		...
		//if ( $checkbox == 'on'){
		//2017-09-13: Richard - Added AC agenda type
		$consent = readPost('consent'.$row["ia_proceedings_id"]);
		$discussion = readPost('discussion'.$row["ia_proceedings_id"]);
		if ( $consent == 'on' || $discussion == 'on'){
			if ($consent == 'on') {
				$agenda_type = 1;
			}
			if ($discussion == 'on') {
...
ADDAPP;*/
			...
			$updateSQL = <<<ADDAPP
				UPDATE ia_proceedings 
				SET application_status_ref = '2', ac_meeting_ref = "{$ac_meeting_id}", lkp_AC_agenda_type_ref = "{$agenda_type}" 
				WHERE ia_proceedings.ia_proceedings_id = {$row["ia_proceedings_id"]}
ADDAPP;
			$errorMail = false;
...
siteSQL;
	...
	while ($srow = mysqli_fetch_array($srs)) {
		...
		//if ( $checkbox == 'on' ) {
		//2017-09-13: Richard - Added AC agenda type
		$consentSite = readPost('consentSite'.$srow["inst_site_app_proc_id"]);
		$discussSite = readPost('discussSite'.$srow["inst_site_app_proc_id"]);
		if ( $consentSite == 'on' || $discussSite == 'on'){
			if ($consentSite == 'on') {
				$agenda_type = 1;
			}
			if ($discussSite == 'on') {
...
UPDATE;*/
			//2017-09-13: Richard - Added AC agenda type
			$updateSQL = "UPDATE inst_site_app_proceedings SET  application_status_ref = '2', ac_meeting_ref = {$ac_meeting_id}, lkp_AC_agenda_type_ref = {$agenda_type}  
				WHERE inst_site_app_proceedings.inst_site_app_proc_id = {$srow["inst_site_app_proc_id"]}";
			$errorMail = false;
			mysqli_query($conn, $updateSQL) or $errorMail = true;
