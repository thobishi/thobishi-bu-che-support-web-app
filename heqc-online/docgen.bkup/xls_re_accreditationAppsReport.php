...
REACCAPP;
	...
	if ($rs){
		...
		while ($row = mysqli_fetch_array($rs)){
			...
			$dash = 'N';
			$check = 'Y';
			$submission_date = ($row["reacc_submission_date"] > '1000-01-01') ? $row["reacc_submission_date"] : $dash;
			$invoice_date = ($pay_invoice_date > '1000-01-01') ? $pay_invoice_date : $dash;
			$checklist_date = ($row["reacc_checklist_date"] > '1000-01-01') ? $row["reacc_checklist_date"] : $dash;
			$a_evals = $app->getSelectedEvaluatorsForApplication($reaccred_id, $criteria, "Reaccred");
			$evaluator_sel = (count($a_evals) > 0) ? $check . ' ' : $dash;
			$evaluation_date = ($row["reacc_evaluation_date"] > '1000-01-01') ? $row["reacc_evaluation_date"] : $dash;
			$site_visit_date = ($row["reacc_sitevisit_date"] > '1000-01-01') ? $row["reacc_sitevisit_date"] : "";
			$secr_recomm_date = ($row["reacc_secretariate_date"] > '1000-01-01') ? $row["reacc_secretariate_date"] : $dash;
			

			$pdate = $row["reacc_deferdue_date"];
			
			$defer_due_date = $row["reacc_deferdue_date"];
//					$defer_due_date = ($row["reacc_deferdue_date"] > '1000-01-01') ? $row["reacc_deferdue_date"] : "&nbsp;";
			$defer_complete_date = ($row["reacc_defercomplete_date"] > '1000-01-01') ? $row["reacc_defercomplete_date"] : "";
			$ac_meeting_date = ($row["reacc_acmeeting_date"] > '1000-01-01') ? $row["reacc_acmeeting_date"] : $dash;
			$cond_due_date = $row["reacc_conditiondue_date"];
//					$cond_due_date = ($row["reacc_conditiondue_date"] > '1000-01-01') ? $row["reacc_conditiondue_date"] : "&nbsp;";
			$cond_met_date = ($row["reacc_conditionmet_date"] > '1000-01-01') ? $row["reacc_conditionmet_date"] : "";
			$repr_submit_date = ($row["reacc_reprsubmit_date"] > '1000-01-01') ? $row["reacc_reprsubmit_date"] : "";
			$repr_complete_date = ($row["reacc_reprcomplete_date"] > '1000-01-01') ? $row["reacc_reprcomplete_date"] : "";
