...
DEFAULT;
		...
		if (mysqli_num_rows($ars) > 0) {
			...
			while ($arow = mysqli_fetch_array ($ars)) {
				...
				$dueStyle = "";
				if ( ($arow["due_date"]!="1970-01-01") && ($arow["due_date"]<=date("Y-m-d")) ) {
					$dueStyle = "CLASS=due";
				}
				if ( ($arow["expiry_date"]!="1970-01-01") && ($arow["expiry_date"]<=date("Y-m-d")) ) {
...
SQL;
						...
						if ($payrs){
							if (mysqli_num_rows($payrs) == 1){  // Only one row shouldbe returned
								...
								if ($payrow['date_invoice'] > '1970-01-01') $inv_date = $check .' ' .$payrow['date_invoice'];
...
$SQL =<<<SQLselect
		SELECT
			...
			IF(r.reacc_submission_date='1970-01-01', "-- Not Submitted --", r.reacc_submission_date) AS reacc_submission_date,
			tmp_reacc.user_ref,
...
SQLselect;
		...
		$SQL = (count($sqlArr) > 0)?($SQL." AND (" . implode(" AND ", $sqlArr).")"):($SQL);
		$SQL = ($is_CHE)?($SQL):($SQL." AND tmp_reacc.processes_ref IN (130)");//$SQL." AND (" . implode(" AND ", $sqlArr).")"
...
TMPREACC;
	...
	$sql =<<<INSREACC
		INSERT INTO tmp_reacc
			...
			WHERE tmp.reacc_applic_id IS NOT NULL AND tmp.reacc_applic_id <> 'NEW'
INSREACC;
...
aSql;
			...
			if ($aRs = mysqli_query($this->getDatabaseConnection(), $aSql))
			{
				...
				while ($row = mysqli_fetch_array($aRs))
				{
					...
					$date_to_SAQA = ($row["date_recommended_to_SAQA"] > "1970-01-01") ? $row["date_recommended_to_SAQA"] : "&nbsp;";
					$SAQA_Id = $row["SAQA_id"];
...
//			$mode_delivery_other = $this->getValueFromTable("Institutions_application", "application_id", $app_id, "mode_delivery_specify_char");
			...
			$mode_delivery .= ($mode_lkp == "5") ? ": ".$mode_delivery_other : "";
			$submission_date = ($row["submission_date"] > '1970-01-01') ? $row["submission_date"] : "Application has not been submitted.";
...
function displayHEQCMeetingDocs($heqc_meeting_id){
	...
	//file_put_contents('php://stderr', print_r("heqc_meeting_id:".$heqc_meeting_id, TRUE));
	$rs = mysqli_query($this->getDatabaseConnection(), $SQL);
...
function buildRegistrarDeclarationForCriterion($appID, $crit_number) {
	...
	if ((isset($_POST['declaration_cancel']) && ($_POST['declaration_cancel']=='OK')) || $declarationDate < '1900-01-01' || $declarationDate == '1970-01-01') {
		$this->setValueInTable ("Institutions_application", "application_id", $appID, $fieldDeclarationSigned, 'No');
		$this->setValueInTable ("Institutions_application", "application_id", $appID, $fieldDeclarationDate, '1970-01-01');
...
PAYMENT;
	...
	*/
	function build_candidacy_search_criteria(){
		...
		if ($s_enddate > ''){
			array_push($fc_arr,"(Institutions_application.submission_date != '1970-01-01' AND Institutions_application.submission_date <= '".$s_enddate."')");
			$this->formFields["subm_end_date"]->fieldValue = $s_enddate;
		}
		...
		if ($s_procenddate > ''){
			array_push($fc_arr,"(ia_proceedings.submission_date != '1970-01-01' AND ia_proceedings.submission_date <= '".$s_procenddate."')");
		...
		if ($inv_enddate > ''){
			array_push($fc_arr,"(payment.date_invoice != '1970-01-01' AND payment.date_invoice <= '".$inv_enddate."')");
		...
		if ($eapp_enddate > ''){
			array_push($fc_arr,"(evalReport.evalReport_date_sent != '1970-01-01' AND evalReport.evalReport_date_sent <= '".$eapp_enddate."')");
		...
		if ($rec_due_enddate > ''){
			array_push($fc_arr,"(ia_proceedings.recomm_access_end_date != '1970-01-01' AND ia_proceedings.recomm_access_end_date <= '".$rec_due_enddate."')");
		...
		if ($ac_enddate > ''){
			array_push($fc_arr,"((Institutions_application.AC_Meeting_date != '1970-01-01' AND Institutions_application.AC_Meeting_date <= '".$ac_enddate."') OR (app.ac_start_date != '1970-01-01' AND app.ac_start_date <= '".$ac_enddate."') OR (iap.ac_start_date != '1970-01-01' AND iap.ac_start_date <= '".$ac_enddate."'))");
			$this->formFields["acmeeting_end_date"]->fieldValue = $ac_enddate;
		}
		...
		if ($heqc_enddate > ''){
			array_push($fc_arr,"(HEQC_Meeting.heqc_start_date != '1970-01-01' AND HEQC_Meeting.heqc_start_date <= '".$heqc_enddate."')");
...
DUE;
		...
		if ($outcome_due_enddate > ''){
			$due_date_end =<<<DUE
				...
				ia_proceedings.condition_short_due_date > '1970-01-01' AND ia_proceedings.condition_short_due_date <= '{$outcome_due_enddate}' OR
				ia_proceedings.condition_prior_due_date > '1970-01-01' AND ia_proceedings.condition_prior_due_date <= '{$outcome_due_enddate}' OR
				ia_proceedings.condition_long_due_date > '1970-01-01' AND ia_proceedings.condition_long_due_date <= '{$outcome_due_enddate}')
DUE;
...
htmlRow;
	...
	function returnReaccAppToInstBeforePayment($reacc_app_id){
		...
		$this->setValueInTable("Institutions_application_reaccreditation", "Institutions_application_reaccreditation_id", $reacc_app_id, "reacc_submission_date", "1970-01-01");
	...
	function notify_finance_percent_complete($mail_subject="",$mail_content="",$finind){
		...
		//if ( $inst_type == 1 && strpos($heqc_ref,'PR') > 0):  
			...
			if ($proc_id > 0 ):
				...
				if ($finind_date == '1970-01-01' && $current_process_status == 0):
					// If an email text is provided then email the user
					if ($mail_subject > "" || $mail_content > ""):
						//$cc = $this->getValueFromTable("users", "user_id", $this->currentUserID, "email");
...
REPORT;
	...
	function generateRecomm ($proc_id) {
		...
		$SQL =<<<REPORT
			...
			WHERE ia_proceedings.ia_proceedings_id = ?
REPORT;
//echo $SQL;
                $conn = new mysqli(DB_SERVER, DB_USER, DB_PASSWD, DB_DATABASE);
                if ($conn->connect_errno) {
...
//echo $SQL;
		$conn = new mysqli(DB_SERVER, DB_USER, DB_PASSWD, DB_DATABASE);
        if ($conn->connect_errno) {
            $this->error_email ("ERROR: DBname", "DBname database down\n\nMySQL: ".$conn->error () ,DB_DATABASE);
            printf("Error: %s\n".$conn->error);
            exit();
        }		
...
REPORT;
			...
			<tr>
				<td>
					<b>HEQC Decision</b>
					<br />The HEQC approved the recommendation of the Accreditation Committee that the <i>$row[program_name]</i> be 
					<br />
					&nbsp;&nbsp;&nbsp; <b>- $heqc_outcome</b>
					<br />
					<br />
...
*/
	...
	while (mysqli_num_rows($rs) > 0 && $row = mysqli_fetch_array($rs)) {
		echo "<tr class='onblue'>";
		echo "<td class='oncolourb' width='20%'>Meeting date:</td>";
...
SITEAPPHEAD;
	...
	switch ($format) {
		case 'sites':
			$sites = "";
			$rs = mysqli_query($this->getDatabaseConnection(), $SQLsites);
			if (mysqli_num_rows($rs) > 0) {
				while ($row = mysqli_fetch_array($rs)) {
...
HTML;
		...
		case 'text':
			...
			break;
		default:
			$rs = mysqli_query($this->getDatabaseConnection(), $SQLinst);
			if (mysqli_num_rows($rs) > 0) {
				$row = mysqli_fetch_array($rs);
				$priv_publ = $this->getValueFromTable("lnk_priv_publ", "lnk_priv_publ_id", $row["priv_publ"], "lnk_priv_publ_desc");
...
SQL;
		$site_apps = array();
		$rs_apps = mysqli_query($this->getDatabaseConnection(), $sql_apps) or die(mysqli_error());
		if ($rs_apps){
...
SITES;
					$rs_sites = mysqli_query($this->getDatabaseConnection(), $sql_sites) or die(mysqli_error());
					if ($rs_sites){
...
HTML;
	...
	function edit_site_recomm($dec_type, $site_proc_id){
		...
		case 'recomm':
			...
			echo "	<td>";
			echo "		<b>Directorate recommendation</b>";
						$this->display_site_outcomes("recomm", $site_proc_id, "int"); 
			echo "	</td>";
			echo "</tr>";
			echo "<tr>";
			echo "	<td>";
...
//HTML;
		...
		case 'ac':
			...
			echo "	<td>";
			echo "		<b>Directorate recommendation</b>";
						$this->display_site_outcomes("ac", $site_proc_id, "int");
			echo "	</td>";
			echo "</tr>";
//HTML;
                        //echo $htmlSix;			
		...
		case 'heqc':
			...
			echo "	<td>";
			echo "		<b>Directorate recommendation</b>";
                                            $this->display_site_outcomes("heqc", $site_proc_id, "int");
			echo "	</td>";
			echo "</tr>";
//HTML;
                        //echo $htmlSeven;			
...
EDIT;
				...
				$progs_sql =<<<PROGS
					...
					ORDER BY Institutions_application.program_name
PROGS;
				$rsp = mysqli_query($this->getDatabaseConnection(), $progs_sql);
...
SQL;
		...
		while ($row = mysqli_fetch_array($rs)){
			$eparr["personNumber"][$row['Persnr']] = $row['Persnr'];
...
//echo "Today: " . $today . " now: " . $now . "<br>";
		...
		$sql =<<<SQL
				...
				FROM payment
				WHERE date_invoice > '1970-01-01'
				AND received_confirmation = 0
				AND date_cancelled = '1970-01-01'
...
//			$this->dbTableInfoArray["payment"] = new dbTableInfo ("payment", "payment_id", $payment_id);
			...
			if ($row["date_first_reminder"] == '1970-01-01'){
...
//echo "First reminder - payment_id: " . $payment_id;
			...
			if ($row["date_final_reminder"] == '1970-01-01'){
				if ($row["date_first_reminder"] > '1970-01-01'){  // first reminder must have been sent
...
//echo "<br><br>Final reminder - payment_id: " . $payment_id;
			...
			if ($may_withdraw == "yes" && $row["date_cancelled"] == '1970-01-01'){
				if ($row["date_final_reminder"] > '1970-01-01'){  // final reminder must have been sent
...
//echo  "<br>Rem3 - max allowed days: " . $rem3_days;
					...
					if ($rem3_outstanding > $rem3_days){
						...
						case "reaccred":
							...
							$this->setValueInTable("Institutions_application_reaccreditation", "Institutions_application_reaccreditation_id", $reacc_id, "submission_date", "1970-01-01");
