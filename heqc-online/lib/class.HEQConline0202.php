...
evalSQL;
             ...
             //echo $row["evalReport.evalReport_date_sent"];
...
$SQL =<<<SQLselect
		SELECT
			...
			tmp_applic1.last_updated,
			CONCAT(name, ': ', users.email) as Process_User,
			tmp_applic1.user_ref,
			processes.processes_desc as Process,
			tmp_applic1.active_processes_id,
		...
		FROM (tmp_applic1, processes, users)
		LEFT JOIN Institutions_application on Institutions_application.application_id = tmp_applic1.application_id
		LEFT JOIN HEInstitution on HEI_id = Institutions_application.institution_id
		LEFT JOIN lkp_process_status on lkp_process_status_id = tmp_applic1.status
		LEFT JOIN payment ON payment.application_ref = Institutions_application.application_id
		WHERE tmp_applic1.processes_ref = processes.processes_id
			AND tmp_applic1.user_ref = users.user_id
