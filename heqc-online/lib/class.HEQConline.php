...
PROCEEDINGS;
	...
	*/
	   function showActiveProcesses ($order="last_updated") {
	   	...
	   	$applicprocesslist = "90,12,40,141,142,7,47,106,112,163,159,160,161,162,165,173,167,168,170,193,198,194,195,203,221,222";
	   	$asql =<<<APPL
	   	SELECT * 
...
OTHER;
		...
		<td class="oncolourb" align="center"><a href="javascript:document.defaultFrm.sortorder.value='p';goto(2);">Process</a></td>
		<td class="oncolourb" align="center">Site application no</td>
		...
		if (mysqli_num_rows($rs) > 0) {
			while ($row = mysqli_fetch_array ($rs)) {
				...
				<tr class='onblue'>
					...
					//echo $arr ;
					if (isset($arr['inst_site_application']) && $arr['inst_site_application']->dbTableCurrentID > 0){
						//$site_application_no ='test';
						$inst_site_app_id = $arr['inst_site_application']->dbTableCurrentID;

						//echo $inst_site_app_id;
						$site_application_no = $this->getValueFromTable("inst_site_application","inst_site_app_id",$inst_site_app_id,"site_application_no");
						echo $site_application_no ;
					}
...
$conn=$this->getDatabaseConnection();
	...
	*/
	...
	//	$fieldValue = (isset($_POST[$html_arr["name"]]))?($_POST[$row[$html_arr["name"]]]):("");
...
//var_dump($unique_flds_array);
		...
		$this->createHTMLGridHeading ($headingArr, 2);
...
SQLselect;
	...
	$SQL = (count($sqlArr) > 0)?($SQL." AND (" . implode(" AND ", $sqlArr).")"):($SQL);
		$SQL = ($is_CHE)?($SQL):($SQL." AND processes_ref IN (5,46, 113, 219)");//$SQL." AND (" . implode(" AND ", $sqlArr).")"
		$SQL .= "GROUP BY 
		HEInstitution.HEI_name,
		Institutions_application.institution_id,
...
SQLselect;
			...
			$SQL = (count($sqlArr) > 0)?($SQL." AND (" . implode(" AND ", $sqlArr).")"):($SQL);
		$SQL = ($is_CHE)?($SQL):($SQL." AND tmp_reacc.processes_ref IN (130, 210, 217)");//$SQL." AND (" . implode(" AND ", $sqlArr).")"
				...
				$admin_action = "&nbsp;";
						if ($this->currentUserID != $row['user_ref'] 						// current user doesn't have the active process - thus no need to take it back.
							&& (($row["processes_ref"] == 130)
								|| ($row["processes_ref"] == 210) || ($row["processes_ref"] == 217))
							&& ($this->currentUserID == $instadm[0]) 						// current user is the administrator
...
aSql;
				...
				//$aSql .=  " ORDER BY a.submission_date, a.AC_Meeting_date ";
...
iframeText;
	...
	***************************************************************************************************************/
	...
	/* this function is to validate any existing email */
...
//echo $aSQL;
	...
	function returnListOfHEQCApplications($application_status, $heqc_meeting=0, $display="plainText") {
		...
		$aSQL = "SELECT a.*, p.ia_proceedings_id 
		FROM Institutions_application a, ia_proceedings p
		WHERE a.application_id = p.application_ref 
...
//			$mode_delivery_other = $this->getValueFromTable("Institutions_application", "application_id", $app_id, "mode_delivery_specify_char");
	...
	function getApplicationInfoTableTopForHEI_sites_v4($app_id) {
		//$app_id = 14736;

		$SQL =<<<APPHEAD
		SELECT CHE_reference_code,  mode_delivery, mode_delivery_specify_char, institution_id, mode_delivery,
		program_name, submission_date,app_version,mode_delivery
		FROM Institutions_application 
		WHERE application_id = $app_id
APPHEAD;
	$SQLmodes =<<<APPHEADmodes
		SELECT lkp_mode_of_delivery_ref,perc_contact,perc_online
...
//			$mode_delivery_other = $this->getValueFromTable("Institutions_application", "application_id", $app_id, "mode_delivery_specify_char");
				...
				echo "<tr class='onblue'>";
				echo "<td class='oncolourb' width='30%' valign='top'>Mode of provisioning:</td> <td>";
				if($row["app_version"]=='5'){
				$rsmode = mysqli_query($this->getDatabaseConnection(), $SQLmodes);
				if (mysqli_num_rows($rsmode) > 0) {

				while ($rowm = mysqli_fetch_array($rsmode)) {
				
				if($rowm["perc_contact"] >0 || $rowm["perc_online"] >0){
								
				if($rowm["lkp_mode_of_delivery_ref"]=='1'){
				echo " Contact </br>";
				}
					
				if($rowm["lkp_mode_of_delivery_ref"]=='2'){
				echo " Blended / Hybrid </br>";
				}

				if($rowm["lkp_mode_of_delivery_ref"]=='3'){
				echo " Distance</br>";
				}
				}
				...
				if($row["app_version"]!='5'){
				if($row["mode_delivery"]=='1'){
								
				echo " Contact</br>";	
				}				
				
				if($row["mode_delivery"]=='2'){
...
SQL;
		...
		$innerRS = mysqli_query($conn, $innerSQL);
...
CHILDSQL;
		...
		$childrs = mysqli_query($conn, $childsql);
...
//2007-01-03: Rebecca - returns a list of the restrictions placed on an AC member
	...
	function getInstitutionName($app_id) {		
		$inst_name = HEQConline::getValueFromTable("HEInstitution", "HEI_id", $app_id, "HEI_name");
		return $inst_name;
	}
...
function scriptGetFormrmv_site($table, $id, $moveto, $dataval="" ){
	
	
	
	$psql =<<<SITEVISIT
	delete from $table  where inst_site_visit_id= $id
...
//				  "evalSelectRequests"
	...
	//if ($app_version == 3 || $app_version == 4 ) {$accForm1 = "accForm1_v3"; (this is how it was before making an edit below)

	//THIS IS FOR VERSION 3 WHIHC IS APP VERSION 4 FOR THE APPLICATION, USE THIS UNTIL NEW VERSION 4.3 IS APPROVED
	...
	//THIS IS FOR VERSION 4.3, NEW APPLICATION WHICH IS APP VERSION 5. USE THIS FOR NEW DEPLOYMENT TO PRODUCTION 
	if ($app_version == 3 || $app_version == 4 ) {$accForm1 = "accForm1_v3";
	...
	{ //$accForm1 = "accForm1_v3";
		
		$forms = array (
			//"accForm3-1_v4","accForm3-2_v4","accForm7_v4_3","accForm6_v4_3","accFormab7_v4","accForm5_v4_3","accForm5_2_v4_3","accForm5_v4","accFormI5_6_V4");
			"accForm1_v4","accForm2_v4","accForm3-1_v4","accForm3-2_v4","accForm7_v4_3","accForm6_v4_3","accFormab7_v4","accForm5_v4_3","accForm5_v4","accFormI5_6_V4");
...
function displayReaccApplicFormOverview ($app_id, $settings){
	...
	if($version ==4){
		...
		$forms = array ("reAccForm2_v4",  "reAccForm6_v4","documentBasket_v4");
...
//echo $SQL;
	...
	function displayRecommUsers($recomm,$label, $recomm_type="application"){
		...
		// $docs_arr = $this->getProceedingDocs($app_proc_id, "application header");
		// $docs = "";
		// foreach($docs_arr as $d){
		// 	$docs .= "<br />" . $d;
		// }
...
//print_r($report);
		...
		function formatRecomm($row){
			...
			<br />The Accreditation Directorate recommends that the <i>{$row["program_name"]} (NQF {$row["NQF_level"]}, {$row["num_credits"]} credits, {$row["lkp_mode_of_delivery_desc"]} mode)</i> be 
			<br />
			&nbsp;&nbsp;&nbsp; <b>- $rec_outcome</b>
			<br />
			<br />
				...
				while ($o_row = mysqli_fetch_array($o_rs)){
					...
					if ($o_row["lkp_condition_term_id"] <> "a" && $o_row["lkp_condition_term_desc"] > ""){
						...
						if ($proc_type == 4){
							if ($table == 'ia_proceedings_recomm_decision'){
								$eval_comment = DBConnect::getValueFromTable("ia_conditions_proceedings","ia_conditions_proceedings_id",$o_row["ia_conditions_proceedings_ref"],"eval_comment");
								$eval_comment= preg_replace('/[^a-zA-Z0-9 \']/', '', $eval_comment);
...
REPORT;
			...
			<br />The Accreditation Committee recommends that the <i>{$row["program_name"]} (NQF {$row["NQF_level"]}, {$row["num_credits"]} credits, {$row["lkp_mode_of_delivery_desc"]} mode)</i> be 
			<br />
			<b> $ac_outcome</b>
			<br />
			<br />
...
REPORT;
		...
		<td>
		<b>HEQC Decision</b>
		<br />The HEQC approved the recommendation of the Accreditation Committee that the <i>{$row["program_name"]} (NQF {$row["NQF_level"]}, {$row["num_credits"]} credits, {$row["lkp_mode_of_delivery_desc"]} mode)</i> be <br />
		<b>$heqc_outcome</b>
		<br />
		<br />
		</td>
...
HTML;
		...
		function create_proceedings($app_id, $proc_type, $app_proc_id,$reacc_id){
			...
			$sql ="INSERT INTO ia_proceedings (
...
REACC;
			...
			$sql =<<<APPSQL
			SELECT Institutions_application.CHE_reference_code,Institutions_application.app_version,
			HEInstitution.priv_publ,
			HEInstitution.HEI_name,
			Institutions_application.program_name,
			NQF_level.NQF_level,
...
APPHEADmodes;
		...
		if ($n_rows > 0){
			...
			switch ($rpt){
				...
				if($rowm["lkp_mode_of_delivery_ref"]=='1'){
				$modeOfDev =$modeOfDev ." Contact </br>";
				}
					
				if($rowm["lkp_mode_of_delivery_ref"]=='2'){
				$modeOfDev =$modeOfDev. " Blended / Hybrid </br>";
				}

				if($rowm["lkp_mode_of_delivery_ref"]=='3'){
				$modeOfDev =$modeOfDev ." Distance</br>";
				}
				}
				...
				$AllmodeOfDev=$modeOfDev;

				}else{
				
				$AllmodeOfDev=$mode_delivery_desc;
...
INSERTB;
				...
				if (!($ac_dec > 0)){
					...
					WHERE ia_proceedings_ref = $app_proc_id
INSERTC;
					$ins_rs = mysqli_query($this->getDatabaseConnection(), $ins_sql) ;
				}
...
SELECT inst_site_visit.institution_ref, HEInstitution.HEI_name, HEInstitution.priv_publ, inst_site_visit.institution_notification_doc,inst_site_visit.initiation_doc,
...
$inst ="";
$attachment = "&nbsp;";
$attachmentinitiation_doc="&nbsp;";
$rowsite="";
$row="";
$Siteapplication ="";
$Notificationletter="";
$initialisationletter="";
$jscripts="";
			...
			if (mysqli_num_rows($rs) > 0) {
				...
				$attachmentsiteapp = "&nbsp;";
				$attsiteapp = new octoDoc($row['siteapp_doc']);					
				
				if ($attsiteapp->isDoc()) {
					$attachmentsiteapp = '<a href="'.$attsiteapp->url().'" target="_blank">'.$attsiteapp->getFilename().'</a>';
				}
...
<tr>
				...
				<tr>
				<td valign='top'>Site application no </td>
				<td class='oncolourb'>$row[site_application_no] </td>
				</tr>
				<tr id="submition_date">					
				<td valign='top'>Submission date</td>
				<td class='oncolourb'>$row[submition_date] </td>
				</tr>
				
				<tr id="attachmentsiteapp">
				<td valign='top'>Site extension or relocation application</td>
				<td class='oncolourb'>$attachmentsiteapp </td>
				</tr>
				<tr id="attachmentinit">
				<td valign='top'>Initiation letter</td>
				<td class='oncolourb'>
HTML;
//echo $SQLsites;
...
$attachmentinitiation_doc = "&nbsp;";
				$attinitiation_doc = new octoDoc($row['initiation_doc']);					
				
				if ($attinitiation_doc->isDoc()) {
					$attachmentinitiation_doc= '<a href="'.$attinitiation_doc->url().'" target="_blank">'.$attinitiation_doc->getFilename().'</a>';
				}

	


$initialisationletter .=<<<HTML
		
		
		$attachmentinitiation_doc<br>
...
var x= hidetr();
document.getElementById("demotr").innerHTML = x;

function hidetr() {


if( '$Datesubmitted' != 'no date'){
document.getElementById("attachmentsiteapp").style.display = '';
   document.getElementById("submition_date").style.display = '';
 document.getElementById("attachment").style.display = 'none';
 document.getElementById("attachmentinit").style.display = 'none';

}else{
document.getElementById("attachmentsiteapp").style.display = 'none';
document.getElementById("submition_date").style.display = 'none';
 document.getElementById("attachment").style.display = '';
document.getElementById("attachmentinit").style.display = '';
...
return '';
				...
				<tr id="attachment">
				<td valign='top'>Notification letter to the institution</td>
				<td class='oncolourb'>
...
$rs1 = mysqli_query($conn, $SQLsites);
if (mysqli_num_rows($rs1) > 0) {
	while ($row = mysqli_fetch_array($rs1)) {
				
				
$att = new octoDoc($row['institution_notification_doc']);
...
var x1= hidetr1();
document.getElementById("demotr").innerHTML = x1;
...
if (mysqli_num_rows($rs) > 0) {
	...
	$sites .=<<<HTML
		...
		$row[site_name], $row[location]: $row[address] <br>
...
$html = $inst .$Siteapplication . $initialisationletter  . $jscripts1 . $Notificationletter . $jscripts . $Siteheader . $sites . '</td></tr>'. '</table >';
...
function getSiteApplicationTableTopEmail($site_app_proc_id, $format="default") {
...
INSERTA;
								$errorMail = false;
								$ins_rs = mysqli_query($this->getDatabaseConnection(), $ins_sqlA) or $errorMail = true;
								$this->writeLogInfo(10, "SQL-INSREC", "class.HEQCOnline.php - DefaultSiteOutcome:"  . $ins_sqlA."  --> ".mysqli_error(), $errorMail);
...
REF;
	...
	function reportAlignedApplications($dateFrom='0',$dateTo='0', $institution='0'){	
		...
		// $aSql ="SELECT HEI_name, CHE_reference_code, program_name AS CHE_programme_name, (SELECT group_concat(old_title) FROM ia_title_history WHERE Institutions_application.application_id = ia_title_history.application_ref GROUP BY application_ref) AS Previous_title , 
		// lkp_mode_of_delivery_desc AS mode_of_delivery, NQF_ref + 4 AS NQF_level, num_credits,	submission_date, 
		// AC_Meeting_date AS Outcome_date, lkp_title AS Outcome, SAQA_id
		// FROM (HEInstitution, Institutions_application)
		...
		// AND Institutions_application.submission_date >= '2009-01-01'
		// AND Institutions_application.AC_desision IN (1,2)
		// AND application_status <> -1";

		//es3
		$aSql ="SELECT HEI_name,Institutions_application.application_id,Institutions_application.institution_id, CHE_reference_code, program_name AS CHE_programme_name, (SELECT group_concat(old_title) FROM 
		ia_title_history WHERE Institutions_application.application_id = ia_title_history.application_ref GROUP BY application_ref) AS Previous_title , 
		lkp_mode_of_delivery_desc AS mode_of_delivery, NQF_ref + 4 AS NQF_level, num_credits,	submission_date, 
		AC_Meeting_date AS Outcome_date, lkp_desicion.lkp_title AS Outcome, SAQA_id, lkp_decwith. lkp_title, CASE WHEN withdrawn_decision_date ='1000-01-01' OR withdrawn_decision_date ='1000-01-01' THEN NULL ELSE withdrawn_decision_date END as withdrawn_decision_date
		FROM (HEInstitution, Institutions_application)
		LEFT JOIN lkp_desicion ON lkp_desicion.lkp_id = Institutions_application.AC_desision
		LEFT JOIN lkp_mode_of_delivery ON lkp_mode_of_delivery_id = Institutions_application.mode_delivery
        LEFT JOIN lkp_desicion AS lkp_decwith ON lkp_decwith.lkp_id = Institutions_application.withdrawn_decision_ref
		WHERE HEInstitution.HEI_id = Institutions_application.institution_id
		AND Institutions_application.submission_date >= '2009-01-01'
		AND Institutions_application.AC_desision IN (1,2)
		AND application_status <> -1";
		...
		if ($institution != '0') {
			array_push($whereArr," HEI_id = '".$institution."' ");
		}


		$aSql .= implode(" AND ", $whereArr);
		$aSql .=  " ORDER BY HEI_name, program_name";
		...
		if ($aRs = mysqli_query($conn, $aSql))
		{
			...
			<td><b>Programme Name</b></td>
			<td><b>HEQC Reference No.</b></td>
			<td><b>HEQSF application</b></td>
			<td><b>Previous Title</b></td>
			<td><b>Delivery Mode</b></td>
			<td><b>NQF Level</b></td>
			<td><b>Number of Credits</b>
			<td><b>Submission date</b></td>
			<td><b>Outcome Date</b>
			<td><b>Outcome</b>
			<td><b>SAQA ID</b></td>
			<td><b>Withdrawn Outcome</b></td>
			<td><b>Withdrawn Decision Date</b></td>";
			...
			while ($row = mysqli_fetch_array($aRs))
			{
					...
					//$applicationLink="";
					$app_doc_basket = "&nbsp;";
					$tmpSettings = "PREV_WORKFLOW=36%7C213&DBINF_HEInstitution___HEI_id=".$institution_id."&DBINF_institutional_profile___institution_ref=".$institution_id."&DBINF_Institutions_application___application_id=".$row["application_id"];
					
					$applicationLink = '<a href="javascript:winPrintApplicationForm(\'Application Form\',\''.$row["application_id"].'\', \''.base64_encode($tmpSettings).'\', \'\');">'.$row["CHE_reference_code"].'</a>';
				
					$app_doc_basket = "&nbsp;";
					$docs_arr = $this->getApplicationDocs($row["application_id"]);
					foreach($docs_arr as $d){
					
 					if(strpos($row["CHE_reference_code"],"HEQSF")!=null)
					{
						$app_doc_basket .= "<br />" . $d;
					}
					}
				...
				<td>".$row["HEI_name"]."</td>
				<td>".$row["CHE_programme_name"]."</td>
				<td>".$applicationLink."</td>
				<td>".$app_doc_basket."</td>
				<td>".$row["Previous_title"]."</td>
				...
				<td>".$row["SAQA_id"]."</td>
				<td>".$row["lkp_title"]."</td>
				<td>".$row["withdrawn_decision_date"]."</td>
				</tr>";
			}
	...
	function reportCanceledApplications($dateFrom='0',$dateTo='0', $institution='0'){	
		...
		$aSql ="SELECT HEI_name, CHE_reference_code, program_name, lkp_mode_of_delivery_desc AS Mode_of_delivery, NQF_ref + 4 AS NQF_level, num_credits AS Credits,
		submission_date, 'Cancelled', date_withdrawn, reason
		FROM HEInstitution, Institutions_application
		...
		if ($dateFrom != '0') {
			array_push($whereArr, ' AND submission_date >= "'.$dateFrom.'"');
		}
	...
	function reportApplicationsInProcess($dateFrom='0',$dateTo='0', $institution='0'){	
		...
		$aSql ="SELECT HEI_name, CHE_reference_code, program_name AS CHE_programme_name, lkp_mode_of_delivery_desc AS mode_of_delivery, HEI_id, NQF_ref + 4 AS NQF_level, num_credits AS Credits,	
		submission_date, (SELECT GROUP_CONCAT( submission_date, ' ') FROM ia_proceedings WHERE ia_proceedings.application_ref = Institutions_application.application_id AND lkp_proceedings_ref IN (1,2,3,4)) AS submission_dates,
		...
		AND Institutions_application.submission_date >= '2009-01-01'
		AND application_status <> -1
		AND Institutions_application.AC_desision NOT IN (1,2,3,4)
		Having in_process > ''
		";
	...
	function reportNotAccreditedApplications($dateFrom='0',$dateTo='0', $institution='0'){	
		...
		$whereArr = array();
				...
				$aSql ="SELECT HEI_name, CHE_reference_code, program_name AS CHE_programme_name, (SELECT group_concat(old_title) FROM ia_title_history WHERE Institutions_application.application_id = ia_title_history.application_ref GROUP BY application_ref) AS Previous_title , 
						...
						WHERE HEInstitution.HEI_id = Institutions_application.institution_id
						AND Institutions_application.submission_date >= '2009-01-01'
						AND Institutions_application.AC_desision IN (3)
						AND application_status <> -1
	...
	function reportAccrediationWithdrawnApplications($dateFrom='0',$dateTo='0', $institution='0'){	
		...
		$aSql ="SELECT HEI_name, CHE_reference_code, program_name AS CHE_programme_name, (SELECT group_concat(old_title) FROM ia_title_history WHERE Institutions_application.application_id = ia_title_history.application_ref GROUP BY application_ref) AS Previous_title , 
		lkp_mode_of_delivery_desc AS mode_of_delivery, NQF_ref + 4 AS NQF_level, num_credits,	submission_date, 
		AC_Meeting_date AS Outcome_date, lkp_dec.lkp_title AS Outcome, SAQA_ID, lkp_decwith. lkp_title, CASE WHEN withdrawn_decision_date ='1000-01-01' THEN NULL ELSE withdrawn_decision_date END as withdrawn_decision_date
,withdrawal_reason
		FROM (HEInstitution, Institutions_application)
		LEFT JOIN lkp_desicion AS lkp_dec ON lkp_dec.lkp_id = Institutions_application.AC_desision
		LEFT JOIN lkp_mode_of_delivery ON lkp_mode_of_delivery_id = Institutions_application.mode_delivery
		LEFT JOIN lkp_desicion AS lkp_decwith ON lkp_decwith.lkp_id = Institutions_application.withdrawn_decision_ref
		WHERE HEInstitution.HEI_id = Institutions_application.institution_id
		AND Institutions_application.submission_date >= '2009-01-01'
		AND withdrawn_decision_ref > 0
		AND Institutions_application.AC_desision IN (1,2)
		AND application_status <> -1
		";
