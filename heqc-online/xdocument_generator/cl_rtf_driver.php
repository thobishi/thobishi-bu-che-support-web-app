		...
		if (!$EndOfTag) {
			...
			if ($this->multilingual_support) {
				...
				switch (strtoupper($this->doc_encoding)) {
				    case "UTF-8": $curr_charset = "1"; break;
...
// ------------------------------------------------------------------------------------------------
	function _iTABLE($attribs,$EndOfTag) {
		if (!$EndOfTag) {
			...
			/////////////
			// 2012-06-06 Robin: php 5.3 upgrade - ereg deprecated
			// Reference: http://www.devthought.com/2009/06/09/fix-ereg-is-deprecated-errors-in-php-53/
			//if (ereg("%",$tbl_width)) {
			if (preg_match("/%/",$tbl_width)) {
				//$yyy = ereg_replace("%", "", $tbl_width);
				$yyy = preg_replace("/%/", "", $tbl_width);
		...
		if ($this->temp_dir !== false) {
			...
			if (preg_match_all("/".$this->image_token."/ms", $this->text,$count_i)) {
				...
				for ($i=0;$i<sizeof($count_i);$i++) {
					...
					$fnd = "/".$this->image_token.$i."nort/";
					$this->text = preg_replace($fnd,$this->image_array[$i],$this->text);
				}
			}
		}
	} // end of function
