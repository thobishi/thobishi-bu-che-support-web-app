...
?>	
		window.opener.parent.addSelectEntries(window.opener.parent.document.defaultFrm.elements["FLDS_resultsSelect[]"], "<?php echo $_GET["userid"] ?>", "<?php echo $uSurname ?>, <?php echo $uName ?>");
	}
</script>
</head>
<body>
...
<?php 
		if ($row["document_url"]>""){
?>
			<a target="_blank" href="<?php echo $path;?>documents/<?php echo $row["document_url"];?>"><?php echo $row["document_name"];?></a></td></tr>
<?php 
		} else {
			echo "&nbsp;";
		}
...
<?php 
		...
		$SQL = "SELECT * FROM SpecialisationLink, CESM_Tree WHERE CESM_code_ref = CESM_code AND Persno_ref=? ORDER BY CESM_code, Description";
