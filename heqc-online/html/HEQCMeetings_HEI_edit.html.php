<?php
$heqc_meeting_id = isset($this->dbTableInfoArray["HEQC_Meeting"]->dbTableCurrentID) ? $this->dbTableInfoArray["HEQC_Meeting"]->dbTableCurrentID : readPost("heqc_meeting_ref");
if ($heqc_meeting_id == 0){
	echo "This meeting cannot be displayed.  Please return to the HEQC Meetings menu option to access the meeting again.";
}
if ($heqc_meeting_id > 0){
	$this->formFields["heqc_meeting_ref"]->fieldValue = $heqc_meeting_id;
...
?>
	...
	<tr>
		<td colspan='10'>
			...
			<?php $this->getHEQCMeetingTableTop($heqc_meeting_id);?>
		</td>
	</tr>
	<tr>
		<td>
...
SQL;
	...
	echo "<td width='15%'>HEQC Outcome</td>";
	echo "</tr>";
	...
	while ($row = mysqli_fetch_array($rs)) {
		...
		$finalEvalDoc = "";
		$outcome = ($row['heqc_board_decision_ref'] > 0) ? $this->getValueFromTable("lkp_desicion", "lkp_id", $row['heqc_board_decision_ref'], "lkp_title") : "";
		$linkToApp = '<a href="javascript:winPrintApplicationForm(\'Application Form\',\''.$row["application_id"].'\', \''.base64_encode($tmpSettings).'\', \'\');">';

		$link1 = $this->scriptGetForm ('ia_proceedings', $row['ia_proceedings_id'], '_label_HEQCminute_edit1');
		echo "<td></td>";
