...
<script>
	function showInfo(uid) {
		URL = "evalSelect.infowin.html.php?userid=";
		URL = URL + uid;
...
//$dbConnect = new dbConnect();
	...
	$post_array = array("Race", "Gender", "Disability", "Province", "Eval_sector_ref", "Organisation_type_ref", "Full_part", "ETQA_ref", "Teaching_experience", "Research_expereince", "qualifications_ref", "employer_ref", "Employer_type_ref", "Auditor", "Evaluator", "National_Review_Evaluator","historical_status_ref","merged_status_ref");
	...
	if (isset($_POST["searchText"]) && ($_POST["searchText"] > "")) {
		...
		array_push($searchArr, "Names LIKE '".$_POST["searchText"]."%' OR Surname LIKE '".$_POST["searchText"]."%'  ");
	}
...
// 2011-09-12 Robin: Ensure only active evaluators can be selected.
//	$SQL = "SELECT Persnr, Names, Surname, Work_Number, E_mail FROM ".implode (", ", $tableArray)." WHERE Evaluator = '1' ";
	
	
	$SQL = "SELECT Persnr, Names, Surname, Work_Number, E_mail FROM ".implode (", ", $tableArray)." WHERE Evaluator = '1' AND active = '2' ";
