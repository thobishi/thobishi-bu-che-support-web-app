...
SITES;
	...
	while ($row = mysqli_fetch_array($rs)){
		...
		$site_visit_rpt = "&nbsp;";
		$site_visit_doc = new octoDoc($row['decision_doc']);
		if ($site_visit_doc->isDoc()) {
			$site_visit_rpt = '<a href="'.$site_visit_doc->url().'" target="_blank">'.$site_visit_doc->getFilename().'</a><br>';
		}
	

            $ac_meeting_ref =  $row['ac_meeting_ref'];
                 $new_AC = $row['ac_meeting_ref'];
             

            $ac_meeting = ($new_AC > "1000-01-01") && ($new_AC >0) ? $this->getValueFromTable("AC_Meeting", "ac_id", $row['ac_meeting_ref'] , "ac_start_date" ) : $row["ac_meeting_ref"] >0;
                  
               $new_HEQC = $row['ac_meeting_ref'];
              $heqc_meeting=($new_HEQC > "1000-01-01") && ($new_HEQC >0) ? $this->getValueFromTable("HEQC_Meeting", "heqc_id", $row['heqc_meeting_ref'] , "heqc_start_date" ) : $row["heqc_meeting_ref"] >0;
		...
		$html_row .= <<<HTMLrow
			<tr>
				...
				<td class="saphireframe">{$row["site_application_no"]}</td>
				<td class="saphireframe">{$apps}</td>
				<td class="saphireframe">{$row["app_type"]}</td>
				...
				<td class="saphireframe">{$site_visit_rpt}</td>
...
HTMLrow;
		...
		<tr class="doveblox">
			...
			<td>Application Type</td>
 			<td>New Site Application Document</td>
