...
<table width=75% border=0 align="center" cellpadding="2" cellspacing="2"><tr>
	...
	<?php	$instProfileFlag = $this->checkInstitutionalProfileContactInfo ($inst_id);
			$instProfileHeadsFlag = $this->checkInstitutionalProfileContactInfoHeads ($inst_id);
	...
	?>
		...
		<table cellpadding="3">
			<tr>
	<?php 
	if (! (( $instProfileFlag) &&  ($instProfileHeadsFlag))) { ?>
...
//$this->validateFields("accForm1_v2");
	...
	//
	$forms = array(array("accForm3-1_v2","PROGRAMME DESIGN: (criterion 1)"),
						array("accForm6_v2","STUDENT RECRUITMENT, ADMISSION AND SELECTION: (Criterion 2)"),
						array("accForm8_1_v2","STAFF QUALIFICATIONS: (Criterion 3)"),
						array("accForm8b_v2","STAFF SIZE AND SENIORITY: (Criterion 4)"),
						array("accForm9_v2","TEACHING AND LEARNING STRATEGY: (Criterion 5)"),
						array("accForm14_v2","ASSESSMENT: (Criterion 6)"),
						array("accForm15_v2","INFRASTRUCTURE AND LIBRARY RESOURCES: (Criterion 7)"),
						array("accForm17_v2","PROGRAMME ADMINISTRATIVE SERVICES: (Criterion 8)"));

	if ($nqf >= 3) {
		array_push($forms, array("accForm19_v2","POSTGRADUATE POLICIES, PROCEDURES AND REGULATIONS: (Criterion 9)"));
