...
?>	
		window.opener.parent.addManager(window.opener.parent.document.defaultFrm.elements["QAmanager"], "<?php echo $uSurname?>, <?php echo $uName?>");
		window.opener.parent.addManager(window.opener.parent.document.defaultFrm.elements["QAmanagerID"], "<?php echo $_GET["userid"]?>");
	}
</script>
</head>
<body>
...
<?php 
		...
		foreach ($cesm AS $key=>$value) {
			if ((substr($key, 2, 2) == "00")) {
				...
				$SQL2 = "SELECT Description FROM CESM_Tree WHERE CESM_code1='".substr($key,0,2)."'";
