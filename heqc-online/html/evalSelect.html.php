...
if (!isset($this->dbTableInfoArray["ia_proceedings"]) || $this->dbTableInfoArray["ia_proceedings"]->dbTableCurrentID == 'NEW'){
	$prog = $this->getValueFromTable("Institutions_application", "application_id", $current_id, "CHE_reference_code");
	die("<br><br>Programme: $prog is missing its proceedings reference.  Please inform Octoplus.");
}
$app_proc_id = $this->dbTableInfoArray["ia_proceedings"]->dbTableCurrentID;
//var_dump($app_proc_id);
$evals = $this->getSelectedEvaluatorsForApplication($app_proc_id,"","Proceedings");
if (count($evals) > 0){
	$this->createAction ("next", "Next", "submit", "", "ico_next.gif");
}

//$proc_id = isset($this->dbTableInfoArray["ia_proceedings"]) ? $this->dbTableInfoArray["ia_proceedings"]->dbTableCurrentID : 0;
?>
...
<tr>
	<td colspan="2">
		...
		<tr>
			<td>
			Please select the evaluators that will evaluate this application.<br><br>
		...
		<tr>
		<td>On the selection of appropriate evaluators rests the strength of the outcome of the Accreditation Phase of the accreditation process.  The Evaluators database will help you choose evaluators for this programme. In making your selection:
		<ul>
...
<tr>
	<td width="50%" valign="top">
		...
		<tr>
			<td align="right"><b>Search:</b> (by Name or Surname (starts with))</td>
...
<!--  2011-09-12 Robin: Evaluators must always be available for selection.  It cannot be optional.  
						Evaluators that have requested to be taken off the list are set to not available.
		<tr>
			<td align="right"><b>Available:</b>&nbsp;</td>
			<td class="oncolour" colspan="3"><?php // $this->showField("active") ?></td>
...
?>
		...
		<tr>
			...
			<hr>
			<br>
			The following evaluators have been selected to evaluate this application.
			</td>
		</tr>
		<tr>
			<td>
			...
			//	array_push($dFields, "type__hidden|name__ia_proceedings_ref|value__".$app_proc_id);
				array_push($dFields, "type__hidden|name__application_ref|value__".$current_id);
				...
				//$this->gridShowRowByRow("evalReport","evalReport_id","application_ref__".$current_id,$dFields,$hFields, 40, 5, "", "true",0);
				//$this->gridShowRowByRow("evalReport","evalReport_id","application_ref__".$current_id,$dFields,$hFields, 40, 5, "", "",0);
				// Changing evaluators to hang off proceedings and not application. Need to update appication ref for the proceedings.
			//	$this->gridShowRowByRow("evalReport","evalReport_id","ia_proceedings_ref__".$app_proc_id,$dFields,$hFields, 40, 5, "", "true",1,"");
				$this->gridShowRowByRow("evalReport","evalReport_id","ia_proceedings_ref__".$app_proc_id,$dFields,$hFields, 40, 5, "", "true",0,"");
...
<script>
	function addGridEntries($keyval){
		console.log($keyval);
	//	document.defaultFrm.cmd.value = "new|evalReport|application_ref|<?php echo $current_id ;?>|Persnr_ref|" + $keyval;
		document.defaultFrm.cmd.value = "new|evalReport|ia_proceedings_ref|<?php echo $app_proc_id ;?>|Persnr_ref|" + $keyval;
		moveto('stay');
	}
</script>
