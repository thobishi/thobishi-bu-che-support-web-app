...
if ($conn->connect_errno) {
    ...
    exit();
			...
			$rs = mysqli_query($conn, $sql);// or die(mysql);
			...
			$html_head = <<<HTMLHEAD
				...
				<tr class="doveblox">
					...
					<td class="doveblox">Withdrawal<br />notice</td>
				</tr>
HTMLHEAD;
			echo $html_head;
			while ($row = mysqli_fetch_array($rs)){
...
SQL;
	 ...
	 //  echo "$ent_sql";
				$ent_rs = mysqli_query($conn, $ent_sql);
				...
				$process = "&nbsp;";
				if ($showProcess == 1){
					// A proceeding payment record will have an ia_proceedings_ref and an application_ref
					if ($app_proc_id > 0){
//echo "app_proc_id = " . $app_proc_id . "<br>";
