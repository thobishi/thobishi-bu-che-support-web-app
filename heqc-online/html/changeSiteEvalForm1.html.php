...
<?php 
	if (isset($_POST["change_eval"]) && ($_POST["change_eval"] > 0)) {
		$SQL = "UPDATE `evalReport` SET do_sitevisit_checkbox=0 WHERE application_ref=? AND Persnr_ref=?";
		...
		$conn = new mysqli(DB_SERVER, DB_USER, DB_PASSWD, DB_DATABASE);
                ...
                $sm = $conn->prepare($SQL);
                $sm->bind_param("ss", $this->dbTableInfoArray["Institutions_application"]->dbTableCurrentID, $_POST["change_eval"]);
                $sm->execute();
                                
		//mysqli_query($SQL);
		echo '<input type="hidden" name="old_eval_id" value="'.$_POST["change_eval"].'">';
	}
?>
