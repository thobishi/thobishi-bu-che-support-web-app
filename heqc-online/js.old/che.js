...
function openFileWin(page,val,field){
	...
	var url = page+'?val='+val+'&field='+field;
	var winStyle = 'scrollbars=yes,toolbar=no,status=no,menubar=no,width='+width+',height='+height+',left='+left+',top='+top;
	var win = window.open(url,'null', winStyle);
}
...
function setData(target){
	document.defaultFrm.data.value = target;
}
...
///vukile trying
//2010-01-05 Robin
function displaySAQAFieldLevel2(value, formObj){
	curMuns = a_fieldlevel2[value];
...
//isaiah and Vukile 2021



//2010-01-05 Robin
function displayFieldLevel2(value, formObj, formObj1){
	curMuns = a_Fieldlevel2[value];
	...
	displaFieldLevel3(value, formObj1)
}


//2010-01-05 Robin
function displaFieldLevel3(value, formObj){

	curMuns = a_saqa_subfield_ref[value];
...
function displayFieldLevel3Defn(){
	idx = document.defaultFrm.FLD_saqa_subfield_ref.selectedIndex;
	//saqa_field_ref saqa_subfield_ref
	if (idx > 0){
		val = document.defaultFrm.FLD_saqa_subfield_ref.options[idx].value;
		document.defaultFrm.saqa_subfield_ref.value = a_saqa_subfield_ref[val];
	}
}
...
$(function() {
	...
	$('#action_deleteProgBtn a').click(function(e){
		e.preventDefault();
		var selectedProgToDelete = new Array();
		$('#activeProcessTable input:checked').each(function() {
			selectedProgToDelete.push($(this).val());
		});
		

		$.ajax({
		 type: 'POST',
		  url: 'pages/deleteProgramme.php',
		  data: { 
				 progArr: selectedProgToDelete
				},
		  success: function(data){
				if(data == "1"){
					document.defaultFrm.GOTO.value = 2;
					document.defaultFrm.submit();
				}else{
					alert("Select the programme(s) that you wish to delete first!!");
				}
	...
	$( "#ViewAllcomment-dialog-form" ).dialog({		
      autoOpen: false,
      height: 700,
      width: 700,
      modal: true,
      ...
      buttons: {
        "Save": function(e) {
			...
			var commentVal = $("#comment").val(),
				application_refVal = $("#application_ref").val(),
				coment_dateVal = $("#coment_date").val(),
				user_refVal = $("#user_ref").val(),
				userNameVal = $("#userName").val(),
				currentProcessVal = $("#currentProcess").val();
			
			allFields.removeClass( "ui-state-error" );
			
			bValid = bValid && checkdialogRegexp( comment, /^\s*\S.*$/, "Comment may not be empty." );
			if ( bValid ) {
				$("#comment_list").html( 
				  commentVal + "<br><small>"+ userNameVal + ", " + currentProcessVal + "," + coment_dateVal + "</small>"
				 );				
				// $( "#comment_table tbody ul" ).append( 
				  // "<li>" + commentVal + "<br><small>"+ userNameVal + ", " + currentProcessVal + "," + coment_dateVal + "</small></li><br>" 
				 // );		 
				 
				$("#view_app_comment").removeClass('hidden');
				allFields.val( "" ).removeClass( "ui-state-error" );
				$( this ).dialog( "close" );
			  
				$.ajax({
				 type: 'POST',
				  url: 'pages/saveOrViewComment.php',
				  data: { 
						  comment: commentVal, 
						  application_ref: application_refVal,
						  coment_date: coment_dateVal,
						  user_ref: user_refVal,
						  userName: userNameVal,
						  command: 'save',
						  currentProcess : currentProcessVal
						},
				  success: function(data){}
				  // async:false
				});
	...
	$('#add_app_comment').click( function(e){
		e.preventDefault();
		$( "#comment-dialog-form" ).dialog( "open" );
	});
	
	$('#view_app_comment').click(function(e){
		e.preventDefault();
		application_refVal = $("#application_ref").val(),
		user_refVal = $("#user_ref").val(),
		$.ajax({
		 type: 'POST',
		  ...
		  success: function(data){
			$( "#ViewAllcomment-dialog-form" ).html(data).dialog( "open" );
		  }
		  // async:false
		});
	...
	function checkdialogRegexp( o, regexp, n ) {
