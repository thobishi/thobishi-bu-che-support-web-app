...
function makeSumProcTable(){
	$SQL2 = "SELECT * FROM active_processes WHERE status=0 AND (due_date <> \"1970-00-00\") AND (due_date < NOW()) AND (expiry_date <> \"1970-00-00\") AND (expiry_date > NOW())";
	$rs2 = mysqli_query($SQL2);
	$SQL3 = "SELECT * FROM active_processes WHERE status=0 AND (expiry_date <> \"1970-00-00\") AND (expiry_date < NOW())";
...
<?php 
	...
	if (mysqli_num_rows($rs) > 0) {
		while ($row = mysqli_fetch_array ($rs)) {
			...
			$dueStyle = "";
			if ( ($row["due_date"]!="1970-00-00") && ($row["due_date"]<=date("Y-m-d")) ) {
				$dueStyle = "CLASS=due";
			}
			if ( ($row["expiry_date"]!="1970-00-00") && ($row["expiry_date"]<=date("Y-m-d")) ) {
...
<?php 
	...
	*/
	...
	function makeLinkTableFileUpload($field, $fake_field="", $table="", $keyFLD="", $keyVal=""){
		...
		$SQL = "SELECT ".$field." FROM ".$table." WHERE ".$keyFLD." = ?";
//echo $SQL.'<br>';
                $sm = $conn->prepare($SQL);
                $sm->bind_param("s", $keyVal);
                $sm->execute();
                $rs = $sm->get_result();
...
WKFSQL;
        $sm = $conn->prepare($wkfSql);
        $sm->bind_param("s", $template);
        $sm->execute();
        $wkfrs = $sm->get_result();
