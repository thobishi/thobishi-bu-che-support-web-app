...
class ChartHelper extends AppHelper {
	...
	function display($name = null, $data = array(), $absolute = false) {
		...
		foreach ($data as $key => $value) {
			...
			switch ($key) {
				case 'data':
					$value = $this->encode($value, $this->encode);
				case 'labels':
				case 'legend':
	...
	function __render($data) {
		...
		if (!isset($data['exitOnError']) || $data['exitOnError'] == false || $this->check_url($this->output)) {
			$graph = $this->Html->image(
					$this->output, $data['html'], array('escape' => false)
			);
	...
	function __setReturn($key, $data) {
		...
		if (isset($this->map[$key]['prefixProperty']) && isset($this->{$this->map[$key]['prefixProperty']})) {
			$return .= $this->{$this->map[$key]['prefixProperty']};
		}

		if (isset($this->map[$key]['afterPrefix'])) {
			$return .= $this->map[$key]['afterPrefix'];
		}

		if (isset($this->map[$key]['seperator'])) {
			if (is_array($this->map[$key]['seperator'])) {
				$returnArray = array();
				$multiDim = false;
				foreach ($data as $dataItem) {
					if (is_array($dataItem)) {
						$returnArray[] = implode($this->map[$key]['seperator'][0], $dataItem);
						$multiDim = true;
					} else {
						$returnArray[] = $dataItem;
					}
				}
				if ($multiDim) {
					$return .= implode($this->map[$key]['seperator'][1], $returnArray);
				} else {
					$return .= implode($this->map[$key]['seperator'][0], $returnArray);
				}
			} else {
	...
	function extEncode($values, $min, $max) {
		...
		foreach ($values as $v) {
			$v = (float)$v;
			$v = abs(round(4095 - (($v * $k) + $d)));
			
			if ($v >= 0 && $v <= 4095) {
				$first = substr($extended_table, round(($v) / $size), 1);
				$second = substr($extended_table, ((($v) % $size)), 1);
	...
	private function __extendedEncode($data, $min, $max) {
		$return = array();
		if ($this->isMulti($data)) {
			foreach ($data as $dataItem) {
				$return[] = $this->__extendedEncode($dataItem, $min, $max);
			}
		} else {
			$return = $this->extEncode($data, $min, $max);
		}

		return $return;
	}

	private function __dataSize($data) {
		$maxSize = 0;
		foreach($data as &$dataItem) {
			$size = count($dataItem);
			if($size > $maxSize) {
	...
	private function __padData($data) {
		if ($this->isMulti($data)) {
			$maxSize = $this->__dataSize($data);
			foreach ($data as &$dataItem) {
				$dataItem = array_pad($dataItem, $maxSize, -1);
			}
		}
		return $data;
	}
	...
	function encode($data = array(), $type = 't') {
		$paddedData = $this->__padData($data);
		switch ($type) {
			case 't':
				$return = $paddedData;
				break;
			case 'e':
				$flatArray = Set::flatten($data);
				$return = $this->__extendedEncode($paddedData, min($flatArray)-(min($flatArray)*0.05), max($flatArray)+(max($flatArray)*0.05));
				break;
		}
