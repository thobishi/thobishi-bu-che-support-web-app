...
jQuery.fn.daterangepicker = function(settings){
	...
	var parseValue = function() {
		var $start = rp.find('.range-start');
		var $end = rp.find('.range-end');
		
		if(rp.find('.ui-daterangepicker-specificDate').is('.ui-state-active')){
			$end.datepicker('setDate', $start.datepicker('getDate') ); 
		}
		var rangeA = fDate( $start.datepicker('getDate') );
		var rangeB = fDate( $end.datepicker('getDate') );
		...
		if(rangeInput.length == 2){
			...
			if(rangeA == '') {
				rangeInput.val(options.beforeText + ' ' + rangeB);
			}
			else if(rangeB == '') {
				rangeInput.val(options.afterText + ' ' + rangeA);				
			}
			else {
	...
	var selectDate = function() { 
		...
		options.onChange(rp.find('.range-start').datepicker('getDate'), rp.find('.range-end').datepicker('getDate'));			
	}
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		...
		var dates = rp.find('.range-start, .range-end').datepicker('option', {
				maxDate: options.latestDate,
				minDate: options.earliestDate,
				onSelect: selectDate
		});
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			rp.find('.range-start').saveDateToData().datepicker('setDate', null).hide(400);
			...
			rp.find('.range-start').restoreDateFromData().show(400);
			rp.find('.range-end').saveDateToData().datepicker('setDate', null).hide(400);
			setTimeout(function(){doneBtn.fadeIn();}, 400);
		}
			...
			dates.datepicker('option', {
				onSelect: function(selectedDate ) {
					var $ = jQuery;
					var option = $(this).hasClass('range-start') ? "minDate" : "maxDate",
			...
			//custom date range
				...
				var dateStart = (jQuery(this).data('dateStart') === false) ? null : (typeof jQuery(this).data('dateStart') == 'string') ? Date.parse(jQuery(this).data('dateStart')) : jQuery(this).data('dateStart')();
				var dateEnd = (jQuery(this).data('dateEnd') === false) ? null : (typeof jQuery(this).data('dateEnd') == 'string') ? Date.parse(jQuery(this).data('dateEnd')) : jQuery(this).data('dateEnd')();
