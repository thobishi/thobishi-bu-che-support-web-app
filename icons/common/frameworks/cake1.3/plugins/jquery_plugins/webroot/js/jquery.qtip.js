...
/* Cache window, document, undefined */ (function(window, document, undefined) {
    ...
    (function($) {
        ...
        function QTip(target, options, id, attr) {
            ...
            this.rendered = this.destroyed = this.disabled = this.waiting = this.hiddenDuringWait = this.positioning = FALSE;
        }
        ...
        PROTOTYPE.render = function(show) {
            ...
            this.tooltip = elements.tooltip = tooltip = $('<div/>', {
                ...
                /* ARIA specific attributes */'role': 'alert',
                'aria-live': 'polite',
                'aria-atomic': FALSE,
        ...
        PROTOTYPE.reposition = function(event, effect) {
            ...
            // Check if mouse was the target
                ...
                event = mouse && mouse.pageX && (adjust.mouse || !event || !event.pageX) ? {
                    pageX: mouse.pageX,
                    pageY: mouse.pageY
                } : (event && (event.type === 'resize' || event.type === 'scroll') ? cache.event : event && event.pageX && event.type === 'mousemove' ? event : (!adjust.mouse || this.options.show.distance) && cache.origin && cache.origin.pageX ? cache.origin : event) || event || cache.event || mouse || {};
        ...
        PROTOTYPE.destroy = function(immediate) {
            ...
            function process() {
                ...
                this.options = this.elements = this.cache = this.timers = this.plugins = this.mouse = usedIDs[this.id] = null;
            }
        ...
        CHECKS = PROTOTYPE.checks = {
            builtin: {
                ...
                '^style.classes$': function(obj, o, v) {
                    this.tooltip.attr('class', [NAMESPACE, defaultClass, v, NAMESPACE + '-pos-' + this.options.position.my.abbrev()].join(' '));
                },
        ...
        function showMethod(event) {
            if (this.tooltip.hasClass(disabledClass)) {
                return FALSE;
            }
        ...
        if (!$.ui) {
            ...
            $.cleanData = function(elems) {
                for (var i = 0, elem;
                (elem = $(elems[i])).length && elem.attr(HASATTR); i++) {
                    try {
        ...
        function Tip(qTip, command) {
            ...
            function parseWidth(corner, side, use) {
                ...
                intCss(elems.content, prop) || intCss(corner.y === TOP && elems.titlebar || elems.content, prop) || intCss(tooltip, prop))) || 0;
            }

            function parseRadius(corner) {
                var prop = BORDER + camel(corner.y) + camel(corner.x) + 'Radius';

                return PLUGINS.ie < 9 ? 0 : intCss(corner.y === TOP && elems.titlebar || elems.content, prop) || intCss(elems.tooltip, prop) || 0;
            }
            ...
            function parseColours(elems, corner) {
                ...
                color.fill = css(tip, BG_COLOR) || css(colorElem, BG_COLOR) || css(elems.content, BG_COLOR) || css(tooltip, BG_COLOR) || tip.css(BG_COLOR);
                ...
                update: function(corner, position) {
                    ...
                    // VML (IE Proprietary implementation)
                        ...
                        inner.css({
                            ...
                            !i && $this.html(createVML('stroke', 'weight="' + (border * 2) + 'px" color="' + color.border + '" miterlimit="1000" joinstyle="miter"'));
                        });
                    }
        ...
        PLUGINS.svg = function(api, svg, corner, adjustMethod) {
            ...
            switch (elem.nodeName) {
                ...
                case 'circle':
                    ...
                    position[0], position[1], (elem.rx || elem.r).baseVal.value, (elem.ry || elem.r).baseVal.value,
                    corner);
                    break;
