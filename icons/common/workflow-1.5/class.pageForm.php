...
class pageForm extends createPage {
	...
	function readDefaultActions () {
		...
		{
			$SQL = "SELECT * FROM template_text WHERE template_ref = '".$this->template."' AND text_type_ref = 3";
	...
	function createInput ($name, $type="", $val="", $size="") {
...
//		$this->printVars($obj);
	...
	function getFieldSettingsFromDB ($name) {
		$obj = &$this->formFields[$name];
		if ( ($this->DBname) && isset($this->dbTableCurrent) ) {
			...
			$fields = mysqli_list_fields($this->DBname, $this->dbTableCurrent);
		    ...
		    if (mysqli_field_name ($fields, $i) == $name) {
					...
					if ( strstr(mysqli_field_flags ($fields, $i), "enum") ) {
						...
						while ($row = mysqli_fetch_assoc ($rs)) {
							if ($row["Field"] == $name) {
								$vals = split ("','", substr ($row["Type"], 6, strlen($row["Type"])-8) );
								foreach ($vals as $val) {
