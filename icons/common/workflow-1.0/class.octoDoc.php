<?php
//require_once('class.octoToken.php');
...
class octoDoc {
	...
	public function url () {
		...
		return ("".OCTODOC_DOWNLOADFILE."?file=".$this->documentNumber."&token=".octoToken::create($this->documentNumber));

	}
	 
	public function urlu () {
		if (!$this->documentNumber) return false;
	...
	private function readDocumentAttrib() {
		//file_put_contents('php://stderr', print_r("\nDocumentNumber : ".$this->documentNumber, TRUE));
		if($this->documentNumber && $this->documentNumber != ""){
	     ...
	     //   $sm = $conn->prepare($SQL);
	      ...
	      //  $rs = $sm->get_result();
			...
			$rs = mysqli_query ($conn, $SQL) or $this->documentNumber = false;
